...
2. **git pull 后 schema bump**：先 `pg_dump -Fc` 并在隔离实例恢复校验；v12→v15 由 Scheduler 自动迁移（v15 会清理 Credential legacy metadata 并增加 health/model catalog）。只有 v12 之前、未知结构或账本 checksum 漂移时才停止并按恢复文档处理，禁止直接 `DROP SCHEMA`。
