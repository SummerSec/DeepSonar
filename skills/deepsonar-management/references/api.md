...
| DELETE | /credentials/:id | agents:write | 删除已保存账号；pending/active Job 拒绝（`CREDENTIAL_IN_USE`）；failed/timeout/orphan 可恢复历史不挡删除；活动扫描拒绝（`CREDENTIAL_SCAN_IN_USE`）；吊销 `job_tokens`；不改写历史 Job 快照（#690：不再存在 RoleConfig 凭据绑定） |
