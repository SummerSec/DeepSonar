...
| POST | /credentials/batch-bind | agents:write | 绑定域 API（不是账号 CRUD）：`{credential_id, role_config_ids[], mode: bind\|migrate, source_credential_id?, model?, effect: new_jobs_only\|refresh_pending, idempotency_key}`；运行中 Job 不会被改写 |
