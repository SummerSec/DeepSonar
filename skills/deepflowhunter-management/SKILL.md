...
export DFH_API_TOKEN=dfh_dev_xxxxxxxx_xxxxxxxx   # 见 references/permissions.md
...
```bash
python scripts/dfh-api.py schema openapi      # → OpenAPI 3 JSON（等同 GET /openapi.json）
python scripts/dfh-api.py schema summary      # → 端点 + scope 摘要
python scripts/dfh-api.py schema markdown     # → Markdown 契约（GET /schema.md）

# 或直接 curl
curl -s "$DFH_BASE_URL/openapi.json"
curl -s "$DFH_BASE_URL/schema?format=summary"
```
...
python scripts/dfh-api.py reports get <canvasId>
python scripts/dfh-api.py reports markdown <reportId>     # Markdown 原文
python scripts/dfh-api.py reports sarif <reportId>        # SARIF 原文
python scripts/dfh-api.py reports retry <canvasId>

# Fact 人工验证
python scripts/dfh-api.py nodes verify <nodeId> --status verified [--note "..."]

# 事件注入（幂等 source + event_id）
python scripts/dfh-api.py events push <projectId> --source ci --event-id build-123 --event-type build_done --content "..."
...
python scripts/dfh-api.py credentials list
python scripts/dfh-api.py credentials create --name kimi --provider kimi --secret '...' [--base-url 'https://...']
python scripts/dfh-api.py credentials update <id> --data '{"metadata":{"base_url":"https://ai.example/v1"}}'
python scripts/dfh-api.py credentials rotate <id> --secret '...'
python scripts/dfh-api.py credentials status <id> --status active|disabled
python scripts/dfh-api.py credentials test <id>
