...
const device: BrokerDevice = {
  key: "rig-1",
  model: "Pixel-7",
  state: "device",
  transport: "adb",
};
/** hdc 目标 key 是 `host:port` 或 USB 序列号（#504）；与 adb 共用同一张设备表。 */
const hdcDevice: BrokerDevice = {
  key: "127.0.0.1:8710",
  model: null,
  state: "device",
  transport: "hdc",
...
test("hdc leases bind their transport and never cross-match adb devices", async () => {
  const s = store({ allowedKeys: [hdcDevice.key] });
  // adb 请求拿不到 hdc 设备。
  const crossMatch = await s.acquire(acquireInput, [hdcDevice]);
  assert.equal(crossMatch.ok, false);
  if (!crossMatch.ok) assert.equal(crossMatch.reason, "no_device");

  const granted = await s.acquire({ ...acquireInput, transport: "hdc" }, [
    device,
    hdcDevice,
  ]);
  assert.equal(granted.ok, true);
  if (!granted.ok) return;
  assert.equal(granted.lease.transport, "hdc");
  assert.equal(granted.lease.deviceKey, hdcDevice.key);

  // 未实现的 transport 仍 fail closed（未授权，不可重试）。
  const unsupported = await s.acquire(
    { ...acquireInput, transport: "serial" },
    [device, hdcDevice],
  );
  assert.equal(unsupported.ok, false);
  if (!unsupported.ok)
    assert.equal(unsupported.reason, "unsupported_transport");
});
...
test("acquire only grants whitelisted devices and mints a verifiable short-lived token", async () => {
  ...
  assert.equal(
    await verifyDeviceLeaseToken(
      granted.lease.token,
      "wrong-secret-0123456789abcdefghij",
    ),
    null,
  );
  assert.equal(
    await verifyDeviceLeaseToken(`${granted.lease.token}x`, SECRET),
    null,
  );

  // 名单外的设备与未实现的传输都不发租约（fail closed）。
  const notWhitelisted = await store({ allowedKeys: [] }).acquire(
    acquireInput,
    [device],
  );
  ...
  const shared = await s.acquire({ ...acquireInput, exclusive: false }, [
    device,
  ]);
...
test("one device holds at most one live lease and re-acquire is refused", async () => {
  ...
  assert.equal(
    s.release(first.leaseId, "released").lease?.releaseReason,
    "released",
  );
  assert.equal(
    (await s.acquire({ ...acquireInput, jobId: "job-2" }, [device])).ok,
    true,
  );
...
test("ttl is bounded by the broker ceiling and expiry frees the device", async () => {
  ...
  const granted = await s.acquire({ ...acquireInput, ttlSec: 86_400 }, [
    device,
  ]);
  assert.equal(granted.ok, true);
  if (!granted.ok) return;
  ...
  assert.equal(
    (await s.acquire({ ...acquireInput, jobId: "job-3" }, [device])).ok,
    true,
  );
});
