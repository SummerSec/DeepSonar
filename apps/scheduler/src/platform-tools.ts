...
export function platformToolGuide(toolNames: string[]): string {
  ...
  return [
    "调用规则：直接调用当前 Agent CLI 工具列表中的同名 MCP 工具，并传入 JSON 对象；不要用 shell、curl 或自行写文件代替工具调用；只用普通文本描述决策、发现或摘要同样不算提交。成功响应会返回 `accepted event`；收到 `isError` 时修正参数后重试，不得把失败调用当作已上报。",
