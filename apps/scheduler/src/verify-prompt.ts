...
4. 全量匹配修订的 outcome=refutes 可提案 verdict=refuted；Fact 冲突或预算耗尽由 Scheduler 落库 inconclusive；finding_id 或 subject_revision 不匹配、可修复的 Fact 不足、来源 Job 失败 → verdict=rework，并列出 missing_evidence。outcome 仅 supports|refutes|inconclusive。否定终态仍由 Scheduler fact-first 硬门写入，提案不等于落库。
