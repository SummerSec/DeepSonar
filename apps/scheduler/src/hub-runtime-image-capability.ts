...
const OFFICIAL_HUB_IMAGE_CAPABILITIES: Readonly<Record<string, HubRuntimeImageCapability>> = Object.freeze({
  "deepsonar-base": {
    ...
    tool_summary: "Node 22 + 最小通用 CLI（git/rg/jq 等）",
    not_included: "完整 JDK/Maven/Go/Rust 矩阵、专项浏览器/DB/设备协议、Kali metapackage",
    suited_roles: ["explore", "analyze", "review", "code", "hub_reason", "verify", "report"],
    suited_evidence: ["只读事实", "轻量分析", "缺省 verify"],
    selection_hints: ["无专项工具需求时省略 runtime_image_key", "不要为动态 PoC/APK/CDP/ClickHouse 选 base"],
    capabilities: ["base-cli", "agent-runtime"],
  },
  "deepsonar-audit": {
    purpose: "通用代码审计：读仓与 Agent 自选启发式，产出 Finding",
    tool_summary: "Base + binutils 等审计辅助；不预装决策型 SAST/密钥扫描器",
    not_included: "Semgrep/gitleaks/shellcheck 决策扫描器、Chromium/ClickHouse/adb/hdc",
    suited_roles: ["audit"],
    suited_evidence: ["静态审计 Finding", "源码阅读"],
    selection_hints: ["通用源码审计选 audit", "Chrome/C++ 专项审计选 chrome-audit；移动端选 mobile"],
    capabilities: ["source-audit", "binutils"],
  },
