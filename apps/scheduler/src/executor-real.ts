...
import { listHubModelCatalog } from "./domains/project-model-policy/index.js";
...
每个 intent 可按本轮目标需要附加可选字段 agent_cli / credential_id / model 选择 Agent CLI、Provider 与模型：先分别调用 list_available_agent_clis、list_available_providers 与 list_available_models，原样使用返回的 agent_cli / credential_id / model_id；与 runtime_image_key 在项目已启用集合内可自由组合；省略时平台用项目软缺省/fallback 或 RoleConfig/CLI 回退。并发以 Provider 配额为准。不得提案未启用目录外的值。
...
${graph ? `\n任务画布（YAML）：\n${graph.yaml}` : taskGoal ? `\n任务目标：${taskGoal}` : ""}`;
  ...
  const runtimePlatformHandler = async (context: PlatformRuntimeHandlerContext): Promise<Record<string, unknown>> => {
    ...
    if (operation === "list_available_models") {
      const models = await listHubModelCatalog(sql as never, job.project_id as string);
      return { accepted: true, operation, models };
    }
