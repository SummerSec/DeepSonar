...
function resultContract(
  ...
  if (isHub) {
    return `需要派发时先调用 list_available_roles 获取本轮数据库角色，并调用 list_available_runtime_images 获取可提案镜像；调用 submit_hub_decision 时只允许 complete、intents 或 payload_file 三选一。from 必须填写当前 YAML root_id/fact/finding 的 UUID 值（不要写字段名 root_id、别名或占位符），role 必须原样命中工具结果（英文 name，禁止缩写），每个 intent 的 description≥8、prompt≥32 且完整自包含。intent 可选 runtime_image_key 必须原样来自本轮 list_available_runtime_images 的 image_key，且属于该条 compatible_agent_clis 中目标角色所用 CLI；省略时按角色缺省镜像解析。目录之外或 CLI 不兼容的 key 会以 invalid_runtime_image 整单拒绝，不要当成 HANDLER_FAILED。本机缺层时决策仍会 accepted，但 Hub 会进入 waiting_human，不要再 mark_job_done。findings_index 中 verify_required=false 的 Finding 已被 minVerifySeverity 策略豁免，不得为它派 review/test 或 request_human。多意图/长 prompt 时必须先 Write 完整 JSON 到 /workspace（如 hub_decision_payload.json），再 submit_hub_decision({"payload_file":"hub_decision_payload.json"})，禁止在 tool 参数里塞超大 JSON 导致截断。submit_hub_decision 每个 Job 成功提交后只能一次；仅当上一次 HTTP 请求失败或参数校验失败时才可修正参数后重试。随后调用 mark_job_done 提交本轮摘要。只在文本里写出决策内容不等于提交，平台只认工具调用。`;
  }
  if (isVerify) {
...
export async function executeReal(
  ...
  // 与角色目录同理：仅 Hub 加载本轮可选运行镜像目录；Worker 调用得到空集。
  const availableRuntimeImageCatalog = canSubmitHubDecision
    ? await listHubRuntimeImageCatalog(sql, job.project_id as string)
    : [];
...
emit_finding 必须遵守以上范围；Scheduler 会校验 profile、重算受支持 CVSS 分数并拒绝冲突输入。`;
  ...
  if (isHub) env.DEEPSONAR_AVAILABLE_RUNTIME_IMAGES_JSON = JSON.stringify(availableRuntimeImageCatalog);
...
每个 intent 可按本轮目标需要附加可选字段 runtime_image_key 选择运行镜像：先调用 list_available_runtime_images 查询本项目已启用、可信且至少一种治理 CLI 能跑的镜像目录，原样使用返回的 image_key，并确认该条 compatible_agent_clis 包含目标角色 CLI（例如需要动态复现选 Kali 类、移动端目标选 mobile 类）；省略该字段时平台按角色缺省镜像解析。不得填写目录之外的 key、OCI 地址或 digest。CLI 不兼容或不在目录中会返回 invalid_runtime_image，应改 key 重试。本机缺层时决策 accepted 但 Hub 进入 waiting_human，不要再 mark_job_done。
...
runtime_image_key 只能原样使用 list_available_runtime_images 本轮返回的、且与目标角色 CLI 兼容的 image_key；目录之外或不兼容的值会使整次决策以 invalid_runtime_image 被拒绝。
...
${graph ? `\n任务画布（YAML）：\n${graph.yaml}` : taskGoal ? `\n任务目标：${taskGoal}` : ""}`;
  ...
  const componentManifest = {
    ...
    shared_assets: {
      ...
      readonly_mount: SHARED_ASSETS_READONLY_ROOT,
  ...
  try {
    ...
    host,
    {
      ...
      model: provider === "pi"
        ? qualifyPiModelRef(model, runtimeConfigFiles, resolvePiPreferredProvider({ model, settingsConfig: snapshot.settings_config_json }))
        : model,
      reasoning,
      dshProvider,
  ...
  // An accepted request_human is the terminal close. Persist it even when a
  // later exclusive tool or runner error would otherwise fail the Job.
