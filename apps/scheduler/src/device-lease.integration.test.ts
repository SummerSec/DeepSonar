...
if (testDatabaseUrl) {
  test("device lease lifecycle is authorized, exclusive, audited and reaped", async () => {
      ...
      request.on("end", () => {
        ...
        if (request.url === "/lease/acquire") {
          ...
          response.writeHead(201).end(
            JSON.stringify({
              lease_id: randomUUID(),
              device_key: String(body.device_key ?? "rig-1"),
              model: "Pixel-7",
              transport: "adb",
              endpoint: { transport: "adb", host: "127.0.0.1", port: 5037, env: {} },
              token: "v1.fake-payload.fake-signature",
              expires_at: new Date(Date.now() + leaseTtlSec * 1000).toISOString(),
            }),
          );
          return;
        }
    ...
    try {
      ...
      const insertJob = async (targetProjectId: string, targetCanvasId: string, roleName: string) => {
      ...
      const requirement = { transport: "adb" as const, key: "rig-1", exclusive: true, ttl_sec: leaseTtlSec, roles: ["test"] };
      const snapshotFor = (roleName: string) => ({ name: roleName, device_requirement: requirement });
      ...
      assert.equal(brokerCalls.filter((call) => call.path === "/lease/acquire").length, 1);
      ...
      assert.equal(JSON.stringify(leaseRow.endpoint_json).includes("fake-signature"), false);
      assert.equal(JSON.stringify(leaseRow.endpoint_json).includes("token"), false);
      const [leasedDevice] = await sql`SELECT status FROM devices WHERE key = 'rig-1'`;
      ...
      await assert.rejects(
        ...
        (error: unknown) => (error as { code?: string }).code === "device_not_available",
      );
      assert.equal(brokerCalls.filter((call) => call.path === "/lease/release").length, 1);
      const [secondLease] = await sql`SELECT count(*)::int AS count FROM device_leases WHERE job_id = ${second.jobId}`;
      ...
      await assert.rejects(
        ...
        (error: unknown) => (error as { code?: string }).code === "device_not_authorized",
      );
      assert.equal(brokerCalls.filter((call) => call.path === "/lease/acquire").length, 2);

      // 5) 终态释放：租约 released、设备回 idle，broker 收到释放。
      assert.equal(await deviceModule.releaseDeviceLeasesForJob(first.jobId, "attempt_terminal"), 1);
      const [released] = await sql`
        SELECT state, release_reason FROM device_leases WHERE job_id = ${first.jobId}`;
      assert.deepEqual(released, { state: "released", release_reason: "attempt_terminal" });
      const [idleDevice] = await sql`SELECT status FROM devices WHERE key = 'rig-1'`;
      assert.equal(idleDevice.status, "idle");
      // 幂等：重复释放不再改动任何行。
      assert.equal(await deviceModule.releaseDeviceLeasesForJob(first.jobId, "attempt_terminal"), 0);
      ...
      const [expired] = await sql`SELECT state, release_reason FROM device_leases WHERE id = ${reaped.leaseId}`;
      assert.deepEqual(expired, { state: "expired", release_reason: "expired" });
      const [idleAgain] = await sql`SELECT status FROM devices WHERE key = 'rig-1'`;
      assert.equal(idleAgain.status, "idle");
      const events = await sql`
        SELECT action FROM device_events WHERE lease_id = ${reaped.leaseId} ORDER BY id`;
      assert.deepEqual(events.map((row) => row.action), ["lease_acquire", "lease_expired"]);
    } finally {
