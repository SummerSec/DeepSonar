...
test("凭据 base_url 已含 /v1 时去掉 Claude Code 重复的版本前缀", () => {
  assert.equal(
    joinGatewayUpstreamUrl("https://relay.example/v1", "v1/messages"),
  ...
  assert.equal(
    joinGatewayUpstreamUrl("https://relay.example/v1/", "/v1/messages", "?beta=1"),
    "https://relay.example/v1/messages?beta=1",
  );
});
