...
test("login rate-limit keys normalize username and IP without leaking empty keys", () => {
  assert.equal(loginRateLimitKey("username", "  Admin "), "admin");
  assert.equal(loginRateLimitKey("username", "ADMIN"), "admin");
  assert.equal(loginRateLimitKey("ip", "  2001:DB8::1  "), "2001:db8::1");
  assert.equal(loginRateLimitKey("username", "   "), "-");
  assert.equal(loginRateLimitKey("ip", ""), "-");
  assert.equal(loginRateLimitKey("username", "a".repeat(200)).length, 128);
});

test("window starts at first failure and rejects the 6th attempt in-window", () => {
  const now = new Date("2026-08-16T12:00:00.000Z");
  const first = evaluateLoginRateLimitWindow(null, 0, now, LOGIN_USERNAME_ATTEMPT_LIMIT);
  assert.equal(first.limited, false);
  assert.equal(first.count, 0);

  const fifth = evaluateLoginRateLimitWindow(now, 5, now, LOGIN_USERNAME_ATTEMPT_LIMIT);
  ...
  const stillLocked = evaluateLoginRateLimitWindow(now, 5, midWindow, LOGIN_USERNAME_ATTEMPT_LIMIT);
  assert.equal(stillLocked.limited, true);
  assert.equal(stillLocked.retryAfterSec, 240);

  const under = evaluateLoginRateLimitWindow(now, 4, midWindow, LOGIN_USERNAME_ATTEMPT_LIMIT);
...
test("expired window resets so attempts work again", () => {
  ...
  const reset = evaluateLoginRateLimitWindow(started, 5, after, LOGIN_USERNAME_ATTEMPT_LIMIT);
