...
export async function resolveAgentSnapshotForJob(
  ...
  if (llm) {
    ...
    const configuredModel = typeof cfg?.model === "string" && cfg.model.trim() ? cfg.model.trim() : PLATFORM_DEFAULT_AGENT_MODEL;
  ...
  const reasoning: ReasoningEffort | null = reasoningRaw === "low" || reasoningRaw === "medium" || reasoningRaw === "high" || reasoningRaw === "xhigh" ? reasoningRaw : null;
  ...
  return {
    ...
    agent_cli: agentCli,
    model: typeof cfg?.model === "string" && cfg.model.trim() ? cfg.model.trim() : PLATFORM_DEFAULT_AGENT_MODEL,
    reasoning,
