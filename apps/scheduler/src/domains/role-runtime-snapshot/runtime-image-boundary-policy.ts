...
- Read /opt/deepsonar/tool-manifest.json and confirm browser.protocol is cdp. Launch only via the image wrappers; required flags include --no-sandbox and --headless=new.
...
- Read /opt/deepsonar/tool-manifest.json. Use git (incl. partial clone), clang, clang-tidy, clangd, llvm, objdump, readelf, llvm-nm as needed.
...
- Read /opt/deepsonar/tool-manifest.json and confirm fuzz.target / fuzz.actual. Use d8, libFuzzer, AFL++, and the preinstalled Clang/LLVM sanitizer toolchain.
...
- Read /opt/deepsonar/tool-manifest.json and confirm clickhouse.official and the pinned version. Prefer image entrypoints (clickhouse-test-env.sh / clickhouse-server.sh).
...
- Read /opt/deepsonar/tool-manifest.json. Use cmake, ninja, clang, clang-tidy, clangd, llvm, objdump, readelf, llvm-nm, python3 as needed.
...
- Read /opt/deepsonar/tool-manifest.json and confirm clickhouse.official and fuzz.target. Prefer image entrypoints and the official clickhouse-local binary.
...
- Read /opt/deepsonar/tool-manifest.json. Use the preinstalled C/C++ audit toolchain and openharmony-audit entrypoints.
...
- Read /opt/deepsonar/tool-manifest.json. Use the preinstalled fuzz/sanitizer toolchain and openharmony-fuzz entrypoints.
