...
import { DeviceRequirement, deviceSandboxEnv } from "@deepsonar/shared-types";
...
export function deviceRequirementFromSnapshot(snapshot: unknown): FrozenDeviceRequirement | null {
  const value =
...
async function projectDeviceAccessEnabled(db: typeof sql, projectId: string): Promise<boolean> {
  ...
  if (!config_json || typeof config_json !== "object" || Array.isArray(config_json)) return false;
  return (config_json as Record<string, unknown>).device_access_enabled === true;
}
...
export async function assertDeviceAccessAuthorized(
  ...
  if (!deviceBrokerConfigured()) {
    throw new DeviceNotAuthorizedError("平台未启用真实设备接入（DEEPSONAR_DEVICE_ENABLED / broker 未配置）");
  }
  if (requirement.transport !== "adb") {
    throw new DeviceNotAuthorizedError(`Phase 1 只支持 adb 设备接入，收到 ${requirement.transport}`);
  }
  ...
  if (!(await projectDeviceAccessEnabled(db, projectId))) {
    throw new DeviceNotAuthorizedError("项目未启用设备接入（device_access_enabled=false）");
  }
}
...
export async function acquireDeviceLeaseForJob(input: {
  ...
  if (!requirement) {
    throw new DeviceNotAuthorizedError("Job 快照里的 device_requirement 不合法");
  }
  ...
  try {
    ...
    await releaseDeviceOnBroker({ leaseId, deviceKey: grant.device_key, reason: "db_rollback" }).catch(() => {});
    if (error instanceof DeviceNotAvailableError || error instanceof DeviceNotAuthorizedError) throw error;
    const code = (error as { code?: string })?.code;
    if (code === "23505") throw new DeviceNotAvailableError("设备已被其他 Job 占用");
 ...
 */
export async function releaseDeviceLeasesForJob(jobId: string, reason: string): Promise<number> {
  const leases = await sql.begin(async (txRaw) => {
    // SAFETY: postgres.js transaction handle exposes the same tagged-template interface as sql.
    const tx = txRaw as unknown as typeof sql;
    const active = await tx<Array<{ id: string; device_id: string; key: string }>>`
  ...
  for (const lease of leases) {
    await releaseDeviceOnBroker({ leaseId: lease.id, deviceKey: lease.key, reason }).catch((error: unknown) => {
      console.error("[device] broker 释放租约失败:", lease.id, error instanceof Error ? error.message : error);
    });
  }
...
export async function reapExpiredDeviceLeases(now: Date = new Date()): Promise<number> {
  const expired = await sql.begin(async (txRaw) => {
    // SAFETY: postgres.js transaction handle exposes the same tagged-template interface as sql.
    const tx = txRaw as unknown as typeof sql;
    const rows = await tx<Array<{ id: string; device_id: string; job_id: string; key: string }>>`
  ...
  for (const lease of expired) {
    await releaseDeviceOnBroker({ leaseId: lease.id, deviceKey: lease.key, reason: "expired" }).catch(() => {});
  }
  return expired.length;
}

/** Job 终态的租约释放；dispatcher / job-control / reaper 共用同一入口。 */
export async function releaseDeviceLeasesForJobQuietly(jobId: string, reason: string): Promise<void> {
  if (!config.device.enabled) return;
  await releaseDeviceLeasesForJob(jobId, reason).catch((error: unknown) => {
    console.error("[device] 释放设备租约失败:", jobId, error instanceof Error ? error.message : error);
  });
}
