...
const grant = {
  ...
  model: "Pixel-7",
  transport: "adb" as const,
  endpoint: { transport: "adb" as const, host: "rig.internal", port: 5037, env: {} },
...
test("broker acquire maps untrusted responses into the two stable device errors", async () => {
  const { DeviceNotAuthorizedError, DeviceNotAvailableError, acquireDeviceOnBroker } = await import(
    "./broker-client.js"
  );
  ...
  try {
    ...
    globalThis.fetch = (async () => respond(403, { error_code: "device_not_authorized" })) as typeof fetch;
    await assert.rejects(
      () => acquireDeviceOnBroker({ jobId: "j", attemptId: "a", projectId: "p", requirement }),
      (error: unknown) => error instanceof DeviceNotAuthorizedError,
    );

    globalThis.fetch = (async () => respond(409, { error_code: "device_not_available" })) as typeof fetch;
    await assert.rejects(
      () => acquireDeviceOnBroker({ jobId: "j", attemptId: "a", projectId: "p", requirement }),
    ...
    globalThis.fetch = (async () => respond(201, { lease_id: "x" })) as typeof fetch;
    await assert.rejects(
      () => acquireDeviceOnBroker({ jobId: "j", attemptId: "a", projectId: "p", requirement }),
      (error: unknown) =>
        error instanceof DeviceNotAvailableError && !error.message.includes("lease_id"),
    );
...
test("release treats an unknown lease as already released and surfaces transport failures", async () => {
  const { DeviceNotAvailableError, releaseDeviceOnBroker } = await import("./broker-client.js");
  const originalFetch = globalThis.fetch;
  try {
...
test("frozen snapshot requirement gates on role and projects only short-lived endpoints", async () => {
  const { deviceRequirementFromSnapshot, jobNeedsDevice } = await import("./application.js");
  ...
  assert.equal(deviceRequirementFromSnapshot({ device_requirement: { transport: "nope" } }), null);
  ...
  assert.deepEqual(Object.keys(env).filter((key) => /API_KEY|SECRET|PASSWORD/u.test(key)), []);
});
