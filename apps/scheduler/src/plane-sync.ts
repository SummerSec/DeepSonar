import { issueContent, PlaneClient } from "@deepsonar/plane-client";
...
export async function planePollOnce(): Promise<number> {
  ...
  for (const p of projects) {
    ...
    try {
      ...
      inc("deepsonar_plane_sync_errors_total");
  ...
  for (const issue of issues) {
    ...
    if (inProgressId) {
      await plane
        ...
        .catch((e) => { inc("deepsonar_plane_sync_errors_total"); console.error("[plane] 状态回写失败:", e); });
    }
    await plane
...
export async function planeWriteback(jobId: string): Promise<void> {
  ...
  await plane
    ...
    .catch((e) => { inc("deepsonar_plane_sync_errors_total"); console.error("[plane] 回写失败:", e); });
  await plane
    .addComment(
      job.plane_project_id,
      job.plane_issue_id,
