  ...
  return (
    <div className="page-scroll">
      ...
      {projects.length ? <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">{projects.slice(0, 6).map((project) => {
        ...
        return <Link key={project.id} to={`/projects/${project.id}/tasks`} className="surface-shell group"><article className="surface-core p-4"><div className="flex items-start gap-4"><div className="theme-chip grid size-10 shrink-0 place-items-center rounded-[14px] text-zinc-400 ring-1"><FolderGlyph /></div><div className="min-w-0 flex-1"><h3 className="truncate text-[13px] font-medium text-zinc-200 transition-colors group-hover:text-[var(--text)]">{project.name}</h3><p className="mt-1 truncate font-mono text-[9px] text-zinc-600">{project.plane_project_id ? "PLANE CONNECTED" : "LOCAL SOURCE"} · {formatTime(project.created_at)}</p></div>{failed > 0 && <Warning size={15} className="text-crit-500" />}</div><div className="mt-5 flex gap-5 font-mono text-[10px] text-zinc-600"><span className={active ? "text-run-400" : ""}>{active} 运行中</span><span>{projectFindings} 发现</span><ArrowUpRight size={13} className="ml-auto transition-transform group-hover:translate-x-1 group-hover:-translate-y-0.5" /></div></article></Link>;
      })}</div> : <EmptyState title="从一个项目空间开始" hint="项目负责长期边界，任务负责一次明确意图。创建项目后即可直接下达第一项审计任务。" action={<Link to={newProjectHref()}><PrimaryButton>创建项目</PrimaryButton></Link>} />}
    </div>
  );
}
