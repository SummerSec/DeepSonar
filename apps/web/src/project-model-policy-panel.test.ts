...
test("项目设置挂载模型允许/缺省/fallback 面板", () => {
  assert.match(settings, /ProjectModelPolicyPanel/);
...
test("设置页从 settings 水合 CLI 白名单与模型策略", () => {
  assert.match(settings, /setEnabledAgentClis/);
  assert.match(settings, /setEnabledModelIds/);
  assert.match(settings, /setFallbackModelIds/);
  assert.match(settings, /model_policy_configured/);
});
