...
const LEGACY_CORAL_THEME_ID = ["open", "kritt"].join("-");
...
export const ACCENT_THEMES = [
  ...
  { id: "coral", label: "珊瑚纸台", caption: "暖白画布 · 珊瑚橙强调", color: "#c04326", surface: "#fbfbfa", scheme: "light" },
] as const;

export type AccentTheme = (typeof ACCENT_THEMES)[number]["id"];

export type AccentThemeStorage = Pick<Storage, "getItem" | "setItem">;

export function resolveAccentTheme(stored: string | null | undefined): AccentTheme {
  const canonical = stored === LEGACY_CORAL_THEME_ID ? "coral" : stored;
  return ACCENT_THEMES.some((theme) => theme.id === canonical) ? canonical as AccentTheme : "mint";
}

export function persistAccentTheme(theme: AccentTheme, storage: AccentThemeStorage = localStorage): void {
  storage.setItem(ACCENT_THEME_STORAGE_KEY, resolveAccentTheme(theme));
}

export function readAccentTheme(storage: AccentThemeStorage = localStorage): AccentTheme {
  const theme = resolveAccentTheme(storage.getItem(ACCENT_THEME_STORAGE_KEY));
