import { Bug, ChartBar, Check, Crosshair, Folder, Gear, MagnifyingGlass, Moon, Palette, Queue, Robot, ShieldCheck, SidebarSimple, Sun, X } from "@phosphor-icons/react";
...
export function AppShell() {
  ...
  return <div className="app-frame">
    ...
    <aside className={`desktop-rail surface-shell ${collapsed ? "is-collapsed" : ""}`}><div className="rail-core surface-core"><div className="brand-lockup"><div className="brand-mark"><ShieldCheck size={20} weight="duotone" /></div><div className="brand-copy"><strong>DeepFlow</strong><span>HUNTER / DFH</span></div><button className="rail-collapse" onClick={() => setCollapsed((value) => !value)} aria-label={collapsed ? "展开导航" : "收起导航"} title={collapsed ? "展开导航" : "收起导航"}><SidebarSimple size={15} weight="light" /></button></div>
