...
export function UsageLedgerBoard({
  ...
  const title = scope === "task" ? "本任务用量账本" : scope === "project" ? "项目用量账本" : "用量账本";
  ...
  return (
    <section className="usage-ledger surface-shell">
                  ...
                  {scope !== "task" && (
                    <UsageTable
                      ...
                      rows={usage.tasks.map((item) => ({
                        ...
                        name: item.title,
                        href: item.canvas_id ? `/projects/${item.project_id}/tasks/${item.canvas_id}` : `/projects/${item.project_id}/tasks`,
...
function UsageTable({
  ...
  rows: Array<{
    ...
    href?: string;
    meta: string;
    tokens: { requests: number; input_tokens: number; output_tokens: number; total_tokens: number };
  }>;
}) {
  return (
    <section>
