...
- **ClickHouse audit/test/fuzz 专项运行时**：三个官方 project-opt-in 镜像对标 Chrome 三件套。Audit 提供 git / CMake / Ninja / Clang-LLVM / binutils；Test 钉死官方 LTS `clickhouse-common-static` `v26.3.28.5`，HTTP 冒烟走 `127.0.0.1`，wrapper 生成沙箱预算内的 config（限制线程池/缓存、禁止 mlock 与 crash report）；Fuzz 装同一官方二进制加 Clang-16 / libFuzzer / AFL++，不从源码编 ClickHouse。三套均要求 `allow_egress=true`，stall 下限与 Chrome 相同。bundled catalog 在正式 `v*` 前保持 `versions: []`。
