/** Pure offline runtime-manual contract shared by admission and workers. */
...
export class RuntimeManualContractError extends Error {
  readonly code = "RUNTIME_MANUAL_CONTRACT";
  constructor(message: string) {
    super(message);
    this.name = "RuntimeManualContractError";
  }
}
...
function invalid(message: string): never {
  throw new RuntimeManualContractError(`runtime manual: ${message}`);
}
...
/** Validate manifest manual metadata. Undefined is not accepted here — callers gate optionality. */
...
export function validateRuntimeManualIndex(value: unknown): RuntimeManualIndex {
  ...
  if (!Array.isArray(raw.entries) || raw.entries.length === 0 || raw.entries.some((entry) => !entry || typeof entry !== "object" || Array.isArray(entry))) {
    invalid("index entries are invalid");
  }
  if (typeof raw.manual_sha256 !== "string" || !/^[0-9a-f]{64}$/.test(raw.manual_sha256)) invalid("index manual_sha256 is invalid");
  if (raw.index_sha256 !== undefined && (typeof raw.index_sha256 !== "string" || !/^[0-9a-f]{64}$/.test(raw.index_sha256))) {
    invalid("index index_sha256 is invalid");
  }
  return {
    contract: RUNTIME_MANUAL_CONTRACT,
