...
expect(chromeFuzzDockerfile.includes("FROM --platform=$BUILDPLATFORM ${BASE_IMAGE} AS v8-build-rootfs") && !chromeFuzzDockerfile.includes("clang_base_path=\"/usr/lib/llvm-16\"") && chromeFuzzDockerfile.includes("target_cpu=arm64") && chromeFuzzPreflight.includes("third_party/llvm-build/Release+Asserts/bin/clang++") && chromeFuzzPreflight.includes("--target=aarch64-linux-gnu") && chromeFuzzPreflight.includes("/usr/bin/ninja") && chromeFuzzPreflight.includes("file -Lb") && chromeFuzzPreflight.includes("libclang_rt[.]builtins"), "Chrome Fuzz arm64 must use V8's pinned x86_64 Clang as a checked cross-toolchain");
