...
`schema.sql` 是全新数据库使用的最新完整基线（当前 v14）。Scheduler 启动时在一个
reserved PostgreSQL session 上持有 session advisory lock：

- 空数据库：原子执行 `database/schema.sql`，直接得到 v14；
- v12 数据库：按 `database/migrations/0013_*.sql`、`0014_*.sql` 的连续编号顺序升级；
- 已是 v14：校验迁移账本与 checksum 后 no-op；
...
当前支持窗口是 **v12 → v14**。v12 的受信冻结 fixture 位于
...
部署升级时直接发布新 Scheduler 并重启；它会在 lock 内完成 v12→v14，其他实例会等待并
在 v14 上 no-op。不要在迁移运行期间删除 volume、手工执行同一 SQL 或修改 migration
...
down migration；从 v14 回退应用代码时仍需保留已新增的结构。
