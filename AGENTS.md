...
- `SandboxRunner` + `RuntimeHost` 是调度器与沙箱之间的唯一接口：`NoopRunner`（骨架）↔ `AgentboxRunner`（过渡实现，`@deepsonar/runtime-sandbox/agentbox`）↔ `OpenSandboxRunner`（#162，绑定 `@alibaba-group/opensandbox@0.1.11`，server/execd/egress 只认 `name@sha256`）。五类 CLI adapter 只依赖内部 process/file 契约，不引用 provider SDK 类型。real 默认仍走 Agentbox；`SANDBOX_PROVIDER=opensandbox` 才启用 OpenSandbox，且不加载 `agentbox-sdk`。重启后续跑走 `ensureHost`。K8s overlay 为 BatchSandbox + Kata（`deploy/opensandbox/config.k8s.toml`）。升级只改显式 pin，禁止 `latest`。换 provider 只动这个包。
