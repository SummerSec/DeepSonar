...
6. Scheduler 在启用 Dispatcher 前同步拉取并校验当前通道（新库默认阿里云 ACR）的 Base digest；失败则启动失败，不带病接单。
...
Scheduler 会同步准备对应 immutable ref；准备失败则配置不落库。Job 运行时 Dispatcher 只检查本地 digest，
