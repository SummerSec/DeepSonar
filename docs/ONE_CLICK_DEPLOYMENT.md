...
沙箱 create/exec/PTY 仍走 `OPEN_SANDBOX_USE_SERVER_PROXY`，不组 overlay。受限网络 / 共享资产卷的 Job 仍只派到 `local` worker（gateway sidecar 仍在控制面 Docker 引擎上）。心跳超时只停止新派发，不自动开新 attempt。派发 claim 与容量预留同一事务完成（#434），create 失败释放占位 lease。
