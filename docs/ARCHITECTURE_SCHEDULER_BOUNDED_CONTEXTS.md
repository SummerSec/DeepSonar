...
| Reaper / reconcile node terminal | Canvas row (`FOR UPDATE`) → `canvas_nodes` → Hub advance (same transaction) | Terminal node paint and Hub idle-wake share the outer Canvas lock; do not write nodes then open a second transaction for Hub. |
