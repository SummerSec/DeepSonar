...
| Dispatcher node upsert / failure close | Canvas row (`FOR UPDATE`) → Job CAS (`failExecution` / provision retry) → `canvas_nodes` / `canvas_edges` → Hub advance | Same canvas-first order as ingest/finalize so the change-log trigger's `canvases` lock is reentrant. Never `UPDATE canvas_nodes` then wait for Canvas. |
