...
> 状态：已在隔离工作树实现，待 review 与部署；当前运行中的 v12 服务/数据库未迁移。
> 验证边界：类型检查、生产构建、静态 smoke 与独立临时 Postgres v13 基线校验已通过；真实广播 E2E、运行数据迁移与发布验收须等现有任务结束后进行。
...
11. 验证至少包含 `pnpm typecheck`、`pnpm build`、现有 convergence/control-MCP smoke、导入导出 round-trip、`git diff --check` 和浏览器实图检查。
