...
- `deepsonar-kali-minimal`（市场名 Kali Test）是 test 与系统 verify 的官方默认镜像：固定官方 `kali-last-release` digest，预装 Python 3.10–3.14、固定 digest 的 Temurin JDK 8/11/17/21、Kali 仓库的 Go/Rust 与清单化审计 CLI；不安装 `kali-linux-*` / `kali-tools-*` metapackage、GUI、桌面或默认工具全集。Python 运行时构建后禁止联网补装，Java/Python 均提供明确的版本化命令。
- Job 创建于 `core.ts` 时按项目 RoleConfig → 全局 RoleConfig → 角色官方默认值解析可信版本（test/verify → Kali Test，audit → Audit，其余 → Base），并立即冻结 digest；Dispatcher/Executor 只消费快照，不在执行期重新解析 tag。
