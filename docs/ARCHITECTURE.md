...
- 执行面节点（#415 P0）：Docker OpenSandbox 不再只认单个 `OPEN_SANDBOX_DOMAIN`。Scheduler 用 Postgres `worker_nodes` / `worker_sandbox_leases` 做注册表与租约；单机启动种子 `local` worker。远程节点持 `DEEPSONAR_WORKER_BOOTSTRAP_TOKEN` 注册并心跳，派发为轮询 + `max_sandboxes`。SDK 流量强制 `useServerProxy=true`。Kubernetes 路径仍走单端点。心跳超时不自动开新 attempt；受限网络/共享资产卷只派 `local`（gateway sidecar 仍在控制面引擎）。`OpenSandboxRunner.destroy` / `destroyResource` 在 sessions 缓存命中后仍走 `client.destroy`，worker-plane 在 `finally` 释放租约；reaper 与 `reconcileOnBoot` 删除终态或缺失 Job 的孤儿租约（#431），容量以租约计数为准。
