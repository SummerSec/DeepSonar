...
- Base、Audit 与 Kali 官方运行时均固定安装 `@earendil-works/pi-coding-agent@0.84.4`，清单记录 npm `sha512` integrity；Docker 构建通过 `npm view ... dist.integrity` 实际核验该摘要后才安装。Pi 的 `models.json` 只写入 Gateway 地址与短期环境变量引用，长期 Provider 密钥不进入快照、工作区或 evidence。
