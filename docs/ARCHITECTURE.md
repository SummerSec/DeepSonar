...
- 移动端专项镜像 `deepsonar-mobile` 为 `project_opt_in`，不改变任何全局默认：Android 提供 JADX CLI、apktool、官方 platform-tools `adb`、mitmproxy（`mitmdump`）、Frida/Objection 与钉死的 `frida-server`；iOS 仅提供 Linux 宿主 `libimobiledevice` / `plistutil` / `iproxy`（无 Xcode / Simulator）；OpenHarmony 应用侧提供 HAP 静态检查（unzip + `pack.info` / `module.json`）与官方 `hdc`（与 `deepsonar-openharmony-test` 同一份 vendor 二进制），不烘焙完整 SDK / DevEco。官方 linux ADB/hdc 为 x86_64；amd64 原生执行，arm64 用同一二进制 + qemu-user-static。不预装 MobSF、jadx-gui、Burp、IDA 或决策扫描器。无 adb / hdc / idevice 目标时必须结构化 `needs_human` / `inconclusive`，禁止用主机反编译或 HAP/IPA 解压叙述冒充设备或流量结果。Ghidra 不在本镜像。OH 源码构建/Clang/fuzz 仍走 `deepsonar-openharmony-*`。
