...
Issue #674 当前模型：运行镜像是平台目录维度，项目不再选择镜像来源（`image_strategy` / `role_runtime_images` 已移除）。Hub 只能提出平台目录中的 `image_key`，Scheduler 在 Job 创建时校验并冻结不可变 digest。项目侧的 `project_runtime_images` 行仍是**可用性硬门，不是兼容残留**：第三方镜像必须 `enabled=true` 才可解析，官方镜像缺行即默认可用、显式 `enabled=false` 可关闭，`selected_version_id` / `pin_policy` 决定该项目跟平台最新还是钉住版本；`readiness.ts` 与 Hub 的 `list_available_runtime_images` 用同一条件过滤，因此项目侧真的会改变 Hub 能提案什么。项目 RoleConfig 不再决定模型、默认 CLI 或镜像（#674），仍可覆盖业务配置（指令、模块、工具开关、沙箱限额等），但不能改变镜像准入。
...
为什么是硬门：`docs/PROJECT_REVIEW_2026-08.md:46` 建议的「文档表征测试」一直未落地，于是 v0.4.x 期间 `AGENTS.md` / `DESIGN.md` 的 schema 主线数字真实漂移了两代（v51 / v53 对代码 v54），另有一批按 `project_managed` 这种已删机制写的 as-built 段落（PR #696 修正）。**已落地的机检是 `pnpm ci:unit:docs-freshness`**，断言四项：schema 主线版本字面量一致；文档引用的 `pnpm` 脚本都存在于 `package.json`；文档相对链接都有目标文件；已删机制字面量只出现在声明解释它们的文档里（白名单在套件内，新增命中即失败）。它已挂进 CI，但覆盖不到语义——「as-built 段落是否与代码相反」必须人工逐条核。
