...
角色能力通过 Capability/Pack 组合声明。Pi 等 CLI 的已注册扩展归属角色能力插件，不属于 RoleConfig 的独立配置；平台准入扩展，Scheduler 校验 CLI/项目范围并将解析后的非敏感引用冻结到 Job。`RoleConfig.pi_extensions` 已删除（#690）。
...
Provider 账号到 RoleConfig 的 `role_credentials` 运行时兼容已删除（#690）。新 Job 以 Provider 能力插件和项目授权解析凭据，不恢复旧绑定。
...
- 不做完整商业级红队平台 / 报告中心
- 不做全局一张超级画布
- 不让任意 Worker 直接操作 Docker / 管理 API / 直接建画布
- 不把所有 CLI 都纳入新 Job 执行路径；当前受治理的 CLI 是 Claude Code、Pi、DSH，Codex/OpenCode 仅保留历史 Session 的只读解析
- 不把多 Scheduler 部署当作默认运行前提；跨副本的持久状态仍由数据库负责

### 成功标准（MVP）

1. 在 Web 或 API 创建一个本地项目与任务，能被系统领取并跑完；绑定 Plane 时，Ready issue 也能进入同一条路径
...
```
┌─────────────────────────────────────────────────────────────┐
│  本地 PostgreSQL + Web（管理真相）                            │
│  Project / Task(Canvas) / RoleConfig / Credential / 规则      │
└──────────────────────────────┬──────────────────────────────┘
                               │ pg_notify / API
┌──────────────────────────────▼──────────────────────────────┐
│  Scheduler（调度与纪律）                                      │
│  claim · 限流 · 状态机 · Reaper · Hub · Verify/Report         │
└──────────────┬───────────────────────────────┬──────────────┘
               │ 起沙箱/跑 Agent                │ 结构化事件
┌──────────────▼──────────┐          ┌─────────▼──────────────┐
│  RuntimeHost             │          │  Canvas / Graph API    │
│  OpenSandbox（real）     │          │  每任务一张画布         │
│  + Claude Code / Pi / DSH│          │  节点 / 边 / 投影 / delta│
└─────────────────────────┘          └────────────────────────┘

        （可选）Plane 集成：Ready issue 输入 + 状态/评论回写
```
...
| 层 | 职责 | 不做 |
|----|------|------|
| **本地库 / Web** | 项目、任务、配置、状态、过程与报告的管理真相 | 不把决策交给 Agent |
| **Plane（可选）** | Ready issue 输入，以及绑定后的状态/评论回写 | 不作为本地身份主键或唯一真相 |
...
> **本地库 / Web = 管理真相；画布 = 过程真相；沙箱 = 执行真相；调度器 = 唯一有副作用的执行者。Plane 只提供可选集成。**
...
```
Project        1 ── * Canvas（Canvas 即任务）
Project        0..1 ── 1 Plane.Project（可选外部绑定）
Canvas         0..1 ── 1 Plane.Issue（可选外部输入）
...
- **每个任务一张画布**（Web/API 创建任务时同事务铸造；同一项目可有多个任务）
- **每次 Job** = 任务画布上的一个执行节点 / 子图，不是新画布
- 当前图使用的节点类型包括：

| node_type | 含义 |
|-----------|------|
| `root` | 任务根（目标、范围与冻结策略） |
| `intent` | Hub 提出的角色意图，与角色 Job 1:1 |
| `job` | 一次运行（角色 / Hub / Verify / Report） |
| `fact` | 工作角色产出的事实 |
| `finding` | 可疑或已确认的问题 |
| `report` | 调度器派生的任务或 Finding 报告 |
| `human` | 人工介入与阻塞 |

`note` 仍是共享节点契约允许的注释类型，但不是 Hub 主闭环的事实节点。
...
- **Reaper**（调度器内置定时任务，默认每 30s 扫描）：
  - 发现 `status=running 且 lease_expires_at < now`，且 `heartbeat_at` 与最近 `events.created_at` 都早于 `lease_expires_at` → 标记 `orphan` → 强制销毁沙箱 → 按重试策略重入队或转 `failed`；若任务绑定 Plane，再尽力回写外部状态。`isAlive` 假阴性停续租时，仍在说话的 Control 心跳 / 语义事件不得被判孤儿。`emit_progress` 在 Job 仍 `running` 时同时刷新 `heartbeat_at` 和 `lease_expires_at`
...
每个可能产生外部副作用的动作都在 `job_attempt_effects` 记录 effect kind/id、intent、输入摘要与 `effect_pending`，观察到结果后写 settlement；所有 effect id/kind、JSON、错误文本均由应用层执行低基数和大小校验。默认 replay policy 为 `never`，进程崩溃或响应不确定时只能收口 `unknown`，不能凭 Promise 或重启自动重放。只有明确声明幂等的销毁动作可使用 `safe`。Dispatcher 观察到 provision 创建失败、Attempt 从未绑定 `sandbox_id`、且超时路径未见到迟到 sandbox handle 时，把该 provision 收成 `settled`（outcome `never_started`），不得留 `unknown`；若超时后迟到 create 已产出 handle（或 late destroy 失败），则 fail closed 收口 `unknown`，禁止再写 `never_started` / `external_effect:false`。`settleAttemptTerminal(failed)` 对仍为 `effect_pending` 且无 `sandbox_id` 的 provision 使用同一 never_started 规则。timeout、cancelled、orphan、interrupted，以及已绑定沙箱后的中断，仍按未知窗口收口 `unknown`。`agent_run` 在 interrupted/orphan 下保持 `unknown`。结算不改写 `replay_policy`。

provision 的 AbortSignal 和 runtime cancel 必须终止外部创建；取消事务先提交 Job/Attempt 终态，随后通知当前进程的 provision 句柄。dispatcher 在调用 provider 前再次校验 Job 仍为 `provisioning`，runtime 在安装 abort listener 后立即重检 signal，避免取消落在句柄注册或监听安装窗口时丢失。资源身份、效果 settlement 与 `jobs.sandbox_id` 在同一个数据库事务落账。Job 终态和 Attempt 终态也在同一事务提交。启动 reconcile 只将“Attempt 为 preparing 且没有效果记录”的 Job 重排；其余 provision 未知窗口标记 orphan，清理容器、共享卷、短期 Token、画布/报告及可选外部集成状态，避免重复创建。
...
5. Scheduler 以可版本化的 Fact-first 策略（`fact_first` v1）做确认硬门：校验 `finding_id`、来源 Job/角色、当前 `subject_revision`、`outcome` 与结构化 `expected`/`actual`；普通文本 verdict 不计。门禁、必需/`advisory` 缺项与确认理由同一路径计算；通过时可直接确认或经 `verify_finding` 提案收口，两条路径结论一致且都不重读源代码、原始制品或 maker 结论。review/test 配对缺口为 advisory，不得与 `confirmed` 同时列为必需缺失。`GraphScope=verify` 仍只投影身份骨架与引用
...
| 模块 | 实现 | 说明 |
|------|------|------|
| **plane-client（可选）** | HTTP Client | 拉取绑定项目的 Issue、改状态、评论；字段映射 |
...
| **agent-harness** | 沙箱内包装脚本 | 调 CLI 协议、接收冻结任务输入、提交 Job 级控制 API、心跳 |
| **web-ui** | React + React Flow (@xyflow/react, MIT) | 项目/任务工作台与画布；按需管理 Plane 集成 |
...
- 队列：PostgreSQL 状态表 + `pg_notify('deepsonar_jobs')` 事件唤醒；轮询默认关闭，仅作为显式运维兜底
...
- 语义事件由沙箱内 Agent 通过短期 capability token 调用 Job 级控制 API 提交，调度器在权威事务中落库：`{ event_id, type, payload }`
  - `type`: `progress` | `fact` | `finding` | `hub_decision` | `done` | `human`
...
- `POST /reconcile/run`（或定时）以本地 `jobs` 表为准；仅在启用 Plane 集成时修正外部状态
...
- 沙箱内不注入调度器数据库、管理 API 凭据或长期 Provider 密钥；`settings_config_json` 的无密钥结构仅在当前 Job 物化为 CLI 配置文件，endpoint 统一改写到 Gateway 并注入短期模型 Job token。Model Gateway 入站认 `Authorization: Bearer` 与 Anthropic `x-api-key` / `anthropic-api-key`（Pi `anthropic-messages` 默认只发后者）；冲突值 fail closed。平台控制 API 只注入另一枚按 operation 限权的短期 token；二者均随终态撤销并随一次性沙箱销毁
...
`projects.config_json.finding_protocol`，任务请求的 `finding_protocol` 只参与创建时合并。`resolveFindingProtocol` 按任务 > 项目 > 全局覆盖（数组按层替换并去重），生成
`EffectiveFindingProtocol` 后写入 `target_json.effective_finding_protocol`；模式只认 `fixed` / `hybrid`（历史 `agent_choice` 与 hybrid 决策相同，启动/导入改写为 hybrid）。后续改设置不改写既有画布或 Job。缺少冻结协议的画布 fail closed，不再现场合并当前全局/项目配置。
...
Credential 独立密钥列使用 AES-GCM；完整 `settings_config_json` 是服务端拥有的 CLI 配置源。管理 API / Web 编辑表单可回显 `settings_config_json` 中的明文 API Key（#689）；Job 快照等非编辑面仍投影为 `[已保存密钥]`。Job 创建时只冻结去除长期密钥后的配置结构；执行器物化 CLI 文件时统一改写为 Gateway endpoint 和短期单 Job token。RoleConfig 的 `env_vars` 仍只能保存非敏感值，调度器数据库、平台 API 凭据和长期 Provider 密钥不下发。
...
并发治理服从单一的六档调度优先级（#568 / #707）：`global_settings.rules_json` 的 effective `maxGlobalJobs`（全局硬 cap） → 项目 `maxConcurrentJobs`（不高于 `maxJobsPerProject`） → Provider 协议 `maxConcurrentByProvider`（设置页可写，相对账号上限的**额外收紧**） → Credential `public_metadata_json.max_concurrent` → 该凭据下 Model ID 的 `model_concurrency` → Agent CLI 全局 `maxConcurrentByAgentCli`。硬约束（方案 A / #658 / #707）：账号 × Agent CLI 排他 1:1，因此凭据级 `max_concurrent` **即**该账号所属 CLI 的并发上限，不必再引入 `max_concurrent_by_agent_cli`；Provider 档先于 Credential 档核对，协议级限流可把同一中转站下多张账号一并收紧。项目可在 `projects.config_json.rules.maxConcurrentJobs` 把本项目 claim 预算收到不高于 `maxJobsPerProject` 的值（`0` 暂停新领取）；未设置则继承全局每项目上限。`.env` 中的 `MAX_GLOBAL_JOBS` / `MAX_JOBS_PER_PROJECT` 仅在全局规则缺失时作为启动默认；项目规则不能放宽全局硬 cap。Provider 与 Agent CLI 上限存于全局规则；Credential 的总上限 `max_concurrent` 和逐模型上限 `model_concurrency` 存于凭据公开元数据。模型可用性只认 Credential `settings_config` 声明的清单，旧 `allowed_model_ids` 字段读写均静默忽略。模型目录由调度器持有密钥并调用 Provider 模型列表接口获取，前端只能接收模型 ID 清单，不能读取长期密钥；Anthropic 兼容子路径按有序候选探测，仅 HTTP 404/405 允许剥离子路径后继续，鉴权、限流、网络、超时与上游错误均立即失败且不读取错误正文。
...
Issue #691 当前模型：运行镜像是平台目录维度，项目不再选择镜像来源（`image_strategy` / `role_runtime_images` 已移除），也不再钉版本。Hub 只能提出平台目录中的 `image_key`，Scheduler 在 Job 创建时校验并冻结不可变 digest。`project_runtime_images.enabled` 只保留收紧动作：官方镜像缺行即默认可用（`deepsonar-base` 永不可停用），其它官方可显式排除；第三方须先由平台写入 `visible_project_ids`，再由项目启用。版本永远跟随平台 channel 最新 trusted。`readiness.ts` 与 Hub `list_available_runtime_images` 同源过滤。项目 RoleConfig 不再决定模型、默认 CLI 或镜像（#674/#697），仍可覆盖业务配置，但不能改变镜像准入。

镜像市场是受治理的 OCI 目录，不是任意容器执行入口。`runtime_images` 表示产品身份（第三方另有 `visible_project_ids` 平台可见范围），`runtime_image_versions` 表示不可变版本，`project_runtime_images` 表示项目排除/启用（无钉版本），`runtime_image_scans` 保留每次准入或复扫证据。
...
- **工具助力，扫描不决策（#267 / #266）**：官方 base / audit / kali / chrome / clickhouse / openharmony / mobile 只预装 **基础工具**（git、ripgrep、jq、file、python/node、binutils；专项镜像另含语言运行时、clang/LLVM、真实浏览器/CDP、d8、官方 ClickHouse、libFuzzer/ASan）。**不**预装 Semgrep / gitleaks / shellcheck 及同类决策扫描器，也 **不** 把平台规定的 `*-scan.sh` / 规则包当作 audit 主路径。Agent 用这些工具自行决定查什么；Finding 质量靠 harness + Verify，不是复现企业 SAST/密钥扫描覆盖面。Job token 与 Provider 密钥仍由运行时精确替换为 `[REDACTED]`，不依赖仓库密钥扫描。
- Base、Audit 与 Kali 官方运行时均固定安装 `@earendil-works/pi-coding-agent@0.85.1`，清单记录 npm `sha512` integrity；Docker 构建通过 `npm view ... dist.integrity` 实际核验该摘要后才安装。Pi 的 `models.json` 只写入 Gateway 地址与短期环境变量引用，长期 Provider 密钥不进入快照、工作区或 evidence。
- Base CI 对 DSH 执行不依赖 Provider 凭据的真实容器启动门禁：完整包闭包统一固定为 `0.1.1-rc.2`，避免 prerelease peer range 混装；无论本轮构建还是复用 `src-*` 镜像，都用平台 standard mode 的完整无 UI Cordis composition、`--network none` 和 packaged-bin 验证 JSON-RPC `initialize` 与干净 `shutdown`。这会让镜像中钉死插件的 Schema 校验全部真实运行；`agent-spine-demo` 的 `toolBash` 必须是对象或 `false`，bash 工具只由 spine 挂载一次；`dsh-subprocess-local` 以固定 integrity 显式安装并先于 `dsh-bash-local` 挂载。
- **镜像体积是准入硬门槛**：按角色拆包、`--no-install-recommends`、不安装重复 Agent SDK/CLI、构建后清理包缓存，并在断网冒烟中以 gzip 压缩分发包检查 `maxSizeMiB`、同时报告解压层大小。重型语言/编译/fuzz 工具链只进入专项镜像，不允许为了“可能用到”扩张默认 base；决策型扫描器不进入任何官方运行时。
- **工具手册是镜像交付物（#588）**：13 个官方镜像都在 `/opt/deepsonar/manuals/` 携带完整离线手册；`index.json` 使用 `deepsonar.runtime.manuals/v1`，并通过 SHA-256 与最终 `tool-manifest.json` 绑定。构建器从同一来源为当前镜像展开公共和专属条目，逐项覆盖最终 Agent 可见工具、包装脚本与能力入口，不能用“参考 base”、官网链接或 `--help` 代替。每个条目说明场景、选型、前置条件、真实命令、输出和证据边界、五类失败处理、组合流程、版本限制、副作用与清理。静态门禁检查 13 镜像和全部入口，镜像冒烟在断网、降权容器内重算摘要并确认文档物理存在。Worker 只得到索引位置并按任务读取章节；手册不扩大 Job 快照、operation allowlist、网络或凭据权限。
- `deepsonar-kali-minimal`（市场名 Kali Test）仅是 test 的官方默认镜像：固定官方 `kali-last-release` digest，预装 Python 3.10–3.14、固定 digest 的 Temurin JDK 8/11/17（默认 17，不含 21）、固定官方 Apache Maven 3.9.16、Kali 仓库的 Go/Rust 等语言运行时；Maven 位于 `/opt/deepsonar/maven` 且不预置 `.m2` 缓存。不安装 `kali-linux-*` / `kali-tools-*` metapackage、GUI、桌面、默认工具全集，也不预装 Semgrep / gitleaks / shellcheck。Python 运行时构建后禁止联网补装，Java/Python/Maven 均提供明确的版本化命令。系统 verify 默认使用最小 Base，需要专项工具时通过 RoleConfig 显式覆盖。
...
- OpenHarmony 专项镜像均为 `project_opt_in`：`deepsonar-openharmony-test`（源码同步与构建，并钉死官方 SDK `toolchains/hdc` 作为设备协议，对标 Chrome Test 的 CDP）、`deepsonar-openharmony-audit`（主机 Clang 静态分析 + ASan/UBSan 工具链，面向 OOB/UAF/提权类假设）、`deepsonar-openharmony-fuzz`（主机 libFuzzer/AFL++ 动态验证）。三者均基于 `deepsonar-base`，不烘焙全量源码、DevEco、完整 SDK 或板级固件；设备可达性靠 `hdc tconn` 或宿主映射，USB 特权不默认打开。无 hdc target 时必须结构化 `needs_human` / `inconclusive`，禁止用主机叙述冒充设备结果。高危挖掘时由项目在「镜像」页显式启用官方专项镜像；角色默认镜像只按全局 RoleConfig / 角色官方默认解析（项目不再持有角色→镜像映射），不改变全局默认。
- 移动端专项镜像 `deepsonar-mobile` 为 `project_opt_in`，不改变任何全局默认：Android 提供 JADX CLI、apktool、Google `bundletool`、EFForg `apkeep`、androguard、钉死的 [droidasc](https://github.com/MG1937/ASC)（Droid ASC：按需 getclass/getmanifest/findrefs，与 JADX 互补；禁止 --gui）、钉死的 [ApkCheckPack](https://github.com/moyuwa/ApkCheckPack)（Agent 可调用的加固/SDK 指纹 CLI，不是平台扫描入口）、官方 platform-tools `adb`、Frida/Objection 与钉死的 `frida-server`；不预装 mitmproxy（挖掘 Job 无法构造 App 流量）；原生 `.so` 用 Debian `binutils`、官方 radare2 与 LIEF，不装 Ghidra / IDA / Cutter；iOS 仅提供 Linux 宿主 `libimobiledevice` / `plistutil` / `iproxy`（无 Xcode / Simulator）；OpenHarmony 应用侧提供 HAP 静态检查（unzip + `pack.info` / `module.json`）与官方 `hdc`（与 `deepsonar-openharmony-test` 同一份 vendor 二进制），不烘焙完整 SDK / DevEco。官方 linux ADB/hdc/ApkCheckPack 为 x86_64；amd64 原生执行，arm64 用同一二进制 + qemu-user-static。不预装 MobSF、jadx-gui、Burp、IDA、决策扫描器或 JADX-AI-MCP / apktool-mcp / FIRERPA 等第三方 MCP。无 adb / hdc / idevice 目标时必须结构化 `needs_human` / `inconclusive`，禁止用主机反编译（含 JADX/droidasc）、ApkCheckPack 指纹或 HAP/IPA 解压叙述冒充设备或流量结果。OH 源码构建/Clang/fuzz 仍走 `deepsonar-openharmony-*`。
- Chrome 专项镜像也全部为 `project_opt_in`，不改变任何全局默认：`deepsonar-chrome-audit` 只提供 git（含 partial clone）、Clang/LLVM 与 binutils 等基础工具，由 Agent 自行选用，不捆绑 Semgrep 规则包或平台固定扫描脚本；`deepsonar-chrome-test` 使用 Debian bookworm-security snapshot 固定版本的 Chromium `151.0.7922.71-1~deb12u1`、Playwright Core 与 CDP，通过受治理的 `--no-sandbox --headless=new` wrapper 启动；`deepsonar-chrome-fuzz` 以固定的 depot_tools/V8 源码提交构建真实 `d8` 与 `v8_json_libfuzzer`，后者从 V8 的 `json_fuzzer` 源集链接 compiler-rt 的 libFuzzer main；V8 `15.1.206.10` 是与浏览器包独立但由 Chromium 151 DEPS 锁定的输入，并提供 clang/lld/compiler-rt/libFuzzer/AFL++。Chrome Fuzz amd64 按正常目标架构构建并执行实际 d8/libFuzzer smoke；arm64 在 x86 runner 上使用固定 Chromium Clang 与 arm64 sysroot 交叉构建，QEMU 仅用于组装，真实 d8/libFuzzer smoke 在 `ubuntu-24.04-arm` 原生 runner 执行，原生 smoke 通过前不得组装发布 index；若任一架构不能生成真实目标则 fail closed，不能用 Node 或 toy harness 冒充。
- ClickHouse 专项镜像同样全部为 `project_opt_in`，不改变任何全局默认：`deepsonar-clickhouse-audit` 提供 git、CMake/Ninja、Clang/LLVM 与 binutils，不捆绑扫描规则或固定扫描入口；`deepsonar-clickhouse-test` 钉死官方 GitHub Release 的未修改 LTS `clickhouse-common-static` `v26.3.28.5`，提供 `clickhouse-local` 与只监听 `127.0.0.1`、数据目录限定 `/workspace/*` 的 `clickhouse-server.sh`（wrapper 现场生成沙箱预算内 config：关 watchdog/mlock/crash report，限制线程池与 mark cache）；`deepsonar-clickhouse-fuzz` 装同一官方二进制，并提供 Clang-16 / compiler-rt / libFuzzer / AFL++。Fuzz **不从源码编译 ClickHouse**（体积/时间不允许，赏金也不认自编包）；sanitizer 重建可用来找洞，交差仍须官方二进制。三套 Job 均要求画布 `allow_egress=true` 以便 clone 源码；stall 下限与 Chrome 相同。核心 CI 不构建；`.github/workflows/clickhouse-runtime.yml` 做 amd64 合同冒烟，Release 按架构原生构建并在组装 index 前完成冒烟。bundled catalog 在正式 `v*` 前保持 `versions: []`，OpenSandbox 官方镜像清单会跳过这些未发布条目，而不是把空版本当成未 pin 失败。
- Job 创建于 `core.ts` 时解析缺省镜像：先取全局 RoleConfig `runtime_image_key`，为空再落到角色官方默认值，然后立即冻结 digest；Dispatcher/Executor 只消费快照，不在执行期重新解析 tag。项目侧不存在 `inherit_global` / `project_managed` 分流（#674 已移除），`projects.config_json` 里的遗留 `image_strategy` / `role_runtime_images` 在读取/写入时物理清扫。
- `image-admission` 是与 Scheduler 进程隔离的 Worker。它对 allowlist registry 的导入执行 digest 解析、可选 Cosign 验签、Syft SBOM、Trivy 漏洞/凭据扫描、ClamAV 恶意文件检查、setuid 枚举和断网硬化自检。官方 catalog 默认免签（digest 信任根）；仅当配置了公钥 `--key` 或 Cosign 3 keyless identity+OIDC issuer 时才 `verify`，缺参不得发出 keyless `verify`。**第三方**扫描通过后仍保持 quarantined，只有 `images:approve` 管理员能提升 trusted；第三方 trusted 复扫失败（含 0 CRITICAL / 0 secret 门禁）自动 revoked。**官方 catalog** 以 Release 清单 digest 为信任根：周期复扫的发行版 CRITICAL/secret 只记失败/告警，不自动吊销。
- 第三方复扫失败的 trusted 版本自动 revoked，调度器/准入 Worker 会取消尚未完成的相关 Job 并精确回收它们的 sandbox ID。官方吊销仅来自管理员或硬失败记录，并写 `audit_logs`。历史 Job 快照、Finding 和扫描记录不删除；新 digest 只进入 quarantined，不自动替换生产版本。Job 冻结选择器在没有 trusted 版本时返回 `RUNTIME_IMAGE_REVOKED` / `RUNTIME_IMAGE_NOT_TRUSTED`，只有确实缺少宿主平台元数据时才是 `RUNTIME_IMAGE_PLATFORM_UNAVAILABLE`。
...
Web 的 `/images` 是独立市场页（可用 `?project_id=` 进入项目排除/启用视图；#691 已移除项目镜像一级 tab）；新建任务仍只接收标题、内容和可选网络策略，不暴露镜像引用。

官方运行时市场只从固定 HTTPS 信任边界内的 GitHub Release `latest` 清单同步。Scheduler 启动时同步一次，并按 `DEEPSONAR_RUNTIME_REGISTRY_SYNC_SEC` 定时刷新；远端不可用时回退随部署内置的清单。正式发布清单存在版本时，环境变量镜像引用仅作为无版本场景的启动兜底，不能覆盖正式最新版本。同步后每个官方镜像只有清单首个版本保持 `promoted_at`，历史版本继续保留，供既有 Job 不可变快照追溯（#691：项目不再钉版本）。Issue #70 Slice B 的 v2 发布清单由 release workflow 以 ACR→GHCR→Docker Hub 顺序生成；每个已发布目的地必须通过真实 `docker buildx imagetools inspect` 并与 canonical digest 相等，`registry_evidence` 记录 inspect/provenance，配置目的地发布失败则清单生成 fail-closed。Slice C 将平台全局 `runtime_registry_channel`（新库默认 `aliyun-acr`）落库：`GET /runtime-images/registry` 返回 `selected_channel`，管理员通过 `PATCH /runtime-images/registry/channel`（`images:manage`）在 `github`、`dockerhub`、`aliyun-acr` 间切换；项目限定 token 被拒绝。选择严格过滤 Scheduler 宿主平台，平台元数据为空也明确 fail closed。real/opensandbox 先提供 `/health` liveness，再后台准备 Base 及全局有效 Audit/Kali 集合与共享资产 helper；inspect 以冻结 digest / image Id 为准，不要求 RepoDigests 等于当前通道仓库名。选定通道 pull 因 timeout/EOF/OSS 失败时，对清单已核实的同 digest 其它通道重试一次，但不改全局 `runtime_registry_channel`、不改写历史 Job 快照；`/health.runtime_images.error` 区分通道超时兜底与 digest 缺失。就绪前不启用 Dispatcher，失败保持 live 并有界退避重试。项目策略/绑定/通道变更缺图时返回 `202 preparing/saved:false`，异步任务完成后重试才提交；通道切换遇到 in-flight 准备也返回当前 `pull-status`（202），不把 `runtime_image_preparation_busy` 当硬失败，同 digest 复用准备锁且通道切换可抢占 `admin_bulk`。拉取任务持久化在 `runtime_image_pull_tasks`；Scheduler 重启把 queued/running 标为 `interrupted`（`error_code=scheduler_restarted`）且不自动 resume。`GET /runtime-images/registry` 附 `image_status`，Hub `list_available_runtime_images` 返回实时 readiness（不含 OCI/digest）；提交未就绪镜像为可重试 `runtime_image_not_ready`。通道门禁覆盖当前平台有效镜像集合，成功前旧通道保持有效。#691 起项目侧不再接受 `version_id` / `pin_policy`；Job 创建时按平台 channel 最新 trusted 解析并冻结 digest。已冻结 Job 快照不改写。Dispatcher 只 inspect 冻结 ref，缺失以 `runtime_image_not_ready` 分类计入指标和失败原因，执行期不 pull。

本地 runtime image GC 只在 real OpenSandbox Docker（非 Kata）且 `DEEPSONAR_RUNTIME_IMAGE_GC_INTERVAL_SEC>0` 时运行。候选必须来自 DB `runtime_image_versions` 与其 ref 账本，且 named immutable ref 的 digest 与版本 digest 一致；保护集合包含所有 `promoted_at` 版本、每产品按时间最近两版（当前 + 上一回滚版）、以及 pending/claimed/provisioning/running/waiting_human Job 快照中的 `runtime_image_version_id`（#691：项目 pin 已移除）。删除前用精确 ancestor 检查全部容器，随后只执行不带 `-f` 的 `docker image rm <known-ref>`；Docker 竞态下的容器引用仍阻止删除。裸 digest、可变 tag、不一致 ref、Docker 检查失败均 fail closed；不调用 `docker image prune` / `docker system prune`，也不处理数据库目录外的服务镜像或第三方资源。
...
项目级表 `project_skill_sources`（#603/#691）只保留显式停用：平台 trusted+enabled 源默认进入本项目 Job 快照可见集合。RoleConfig `modules_json` 为空时不再预装业务 Skill；Agent/Worker 在 Job 内通过 `list_available_skills` / `search_skills` 自主选择，再调用 `pull_skill` 拉取具体 Skill 文件并读取 `SKILL.md`。拉取只允许平台 trusted+enabled 且未被项目停用的源，并由 Job token、操作 allowlist、预算、网络策略和内容 hash 重新校验；Skill 不能扩大 Job 权限。Hub `list_available_skill_sources` 只返回源摘要。平台 `deepsonar-control` Skill 继续强制注入。跨任务业务身份复用写成受治理 Skill，而不是项目级 `agent_roles`。
...
| 任务内容诱导 Agent 指定恶意镜像 | Hub 不输出镜像 ID；Scheduler 只从可信目录解析并冻结 digest；未准入/不在可见范围/已排除的版本无法建 Job |
...
| 项目 Token 自我提权或跨租户写 Job/Transfer/镜像目录（#403） | 新 Token scopes 必须是调用者子集；项目 actor 不能签发全局/跨项目 Token。`POST /jobs` 与 Transfer `target_project_id` 以 actor 项目为准。全局 runtime-image catalog 对项目 actor 拒绝；usage 只投影本项目 |
| 持 bootstrap token 的 worker 改写 endpoint / 抢占 `local` 或任意 node_id（#433） | 保留 id 禁止远程注册；已有节点须管理员 forget 后才能 reclaim；远程 endpoint 走拒绝列表 + 可选 CIDR/hostname allowlist；心跳不能改 kind/所有权；注册与拒绝心跳审计并限流；本地 seed 在身份被改写后轮换 node token |
...
| 敏感信息 | transcript 含客户源码、finding 可能含挖到的硬编码密钥 → 冷存储 at-rest 加密、访问走鉴权端点；Job token / Provider 密钥由运行时精确替换为 `[REDACTED]`（`redactRuntimeSecrets` / evidence 脱敏），不靠对目标仓库跑密钥扫描器 |
...
- **质量指标与 Hub 回放（#445 Phase 1）**：`GET /dashboard/quality` 与项目/任务别名从现有 Job、Finding、Verify、Usage Ledger 只读派生确认率、误报率、Verify 分歧、人工介入与单 Finding 成本；`GET …/quality/replay` 输出冻结 `hub_replay` v1。`/projects/:id/quality*` 由 `projectScopeHook` 与 handler 校验 `req.actor.projectId`，跨项目返回 `PROJECT_MISMATCH`。不改变 Hub 决策，不写 Experience
...
- 运维可将旧库备份后套最新基线，再按列名交集回填（`pnpm db:rebuild`）；回填后只 reset public owned sequences，并断言 `last_value >= MAX(id)`。Scheduler 启动对序列漂移自动 `setval`，对不齐 fail closed
...
为什么是硬门：`docs/PROJECT_REVIEW_2026-08.md:46` 建议的「文档表征测试」一直未落地，于是 v0.4.x 期间 `AGENTS.md` / `DESIGN.md` 的 schema 主线数字真实漂移了两代（v51 / v53 对代码当时版本），另有一批按 `project_managed` 这种已删机制写的 as-built 段落（PR #696 修正）。**已落地的机检是 `pnpm ci:unit:docs-freshness`**，断言四项：schema 主线版本字面量一致；文档引用的 `pnpm` 脚本都存在于 `package.json`；文档相对链接都有目标文件；已删机制字面量只出现在声明解释它们的文档里（白名单在套件内，新增命中即失败）。它已挂进 CI，但覆盖不到语义——「as-built 段落是否与代码相反」必须人工逐条核。
...
具体命令清单与提交前检查见 [`AGENTS.md`](../AGENTS.md) 的「发布前的文档保鲜」一节；可机检部分已由 `pnpm ci:unit:docs-freshness` 在 CI 中阻断。
