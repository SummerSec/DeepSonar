...
- Base CI 对 DSH 执行不依赖 Provider 凭据的真实容器启动门禁：无论本轮构建还是复用 `src-*` 镜像，都用平台 standard mode 的完整无 UI Cordis composition、`--network none` 和 packaged-bin 验证 JSON-RPC `initialize` 与干净 `shutdown`。这会让镜像中钉死插件的 Schema 校验全部真实运行；`agent-spine-demo@0.1.0-rc.6` 的 `toolBash` 省略即默认启用，禁止传不属于 `false | object` union 的 `true`。
