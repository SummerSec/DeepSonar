...
- ClickHouse 专项镜像同样全部为 `project_opt_in`，不改变任何全局默认：`deepsonar-clickhouse-audit` 提供 git、CMake/Ninja、Clang/LLVM 与 binutils，不捆绑扫描规则或固定扫描入口；`deepsonar-clickhouse-test` 钉死官方 GitHub Release 的未修改 LTS `clickhouse-common-static` `v26.3.28.5`，提供 `clickhouse-local` 与只监听 `127.0.0.1`、数据目录限定 `/workspace/*` 的 `clickhouse-server.sh`；`deepsonar-clickhouse-fuzz` 装同一官方二进制，并提供 Clang-16 / compiler-rt / libFuzzer / AFL++。Fuzz **不从源码编译 ClickHouse**（体积/时间不允许，赏金也不认自编包）；sanitizer 重建可用来找洞，交差仍须官方二进制。三套 Job 均要求画布 `allow_egress=true` 以便 clone 源码；stall 下限与 Chrome 相同。核心 CI 不构建；`.github/workflows/clickhouse-runtime.yml` 做 amd64 合同冒烟，Release 按架构原生构建并在组装 index 前完成冒烟。
