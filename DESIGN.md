...
| 长期从 Agentbox 迁移到 OpenSandbox | #162 | **Phase 1 已落地，Phase 2 adapter 可测**：`packages/runtime-sandbox` 现有 provider-neutral `RuntimeHost` / `RuntimeProcess` / `RuntimeResource`；五类 CLI（含 Pi / DSH）只依赖该契约，OpenSandbox 不按 CLI 分流。`OpenSandboxRunner` 用注入的 client 做 Docker PoC（`SANDBOX_PROVIDER=opensandbox`），硬限制缺失 fail closed，`none/restricted/egress` 映射到 networkPolicy，TTL 由 Scheduler 持有。OpenSandbox 升级只接受显式 SDK 版本与 `name@sha256` pin，禁止 `latest`。real 默认仍走 Agentbox 实现这些契约。Phase 3 Kubernetes/Kata 与 Phase 4 删除 Agentbox 仍待全部门禁。 |
