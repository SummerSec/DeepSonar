...
| 非 root 专项镜像 Gateway hosts bind | #346 | **已完成**：Chrome / ClickHouse / OpenHarmony / Mobile 专项镜像保持 `USER deepsonar`。OpenSandbox SDK 0.1.11 不能在 create 时注入 ExtraHosts；provision 改为宿主 `docker exec -u 0` / `kubectl exec -u 0` 写入 `/etc/hosts`，guest execd 只做只读校验。失败文案含 `as root` / Gateway hosts bind，避免 Worker 把 provision 失败误报成缺 Chromium 或官方 ClickHouse ELF。官方 ELF 仍只在 `deepsonar-clickhouse-test/fuzz`，不进 `deepsonar-base`。 |
