...
| OpenHarmony Test 钉死官方 hdc | #268 | **已落地**：`deepsonar-openharmony-test` 从官方 OpenHarmony public SDK `toolchains/hdc`（及 `libusb_shared`）钉死设备协议，URL + SHA256 写入 `openharmony-test-runtime.json`；amd64 原生执行，arm64 用同一官方 linux-x64 二进制 + qemu-user-static。冒烟只要 `hdc version` / `hdc -v`，CI 不要求真机。无 target 时结构化 `needs_human` / `inconclusive`。不装 DevEco / 完整 SDK / HarmonyOS 闭源工具链，不默认打开 nmap 或 USB 特权。audit/fuzz 主机 Clang/ASan/libFuzzer 保留，不把 gdb/strace 抄成 OH 设备协议。 |
