...
| Provider 账号删除 | #180 | **已完成**：`DELETE /credentials/:id`（`agents:write`）。pending/active Job 拒绝（`CREDENTIAL_IN_USE`）；仍绑定 RoleConfig 时需 `?unbind=true`（`CREDENTIAL_BOUND`）。同一事务吊销并删除 `job_tokens`，不改写历史 Job 快照；审计不含密文。Web 凭据列表提供确认删除。 |
