...
| resume 后旧 human 毒化新 Attempt | #298 | **已完成**：`assertTerminalEventHistory` 只统计当前 active Attempt 之后的终态事件；resume-frozen / 新 Attempt 可 `mark_job_done` / `submit_hub_decision`。同 Attempt 的 `request_human` 与 done/hub 仍互斥。 |
| 人工收尾同回合 job_not_running | #300 | **已完成**：同一摄入事务用 lock 时 Job 状态做 running 守卫；`hub_decision` 先于 `done` 落地；本回合已成功终态后的迟到 `mark_job_done` 幂等，不整笔回滚。开始前已终态的迟到回调仍 `job_not_running`。 |
