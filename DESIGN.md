...
| sandbox 创建失败（egress sidecar 宿主端口撞 Windows 排除区）不自动重试 | #332 | **已立项**：real 模式并发派 Job 时，OpenSandbox 为 restricted 沙箱建 egress sidecar 随机分配的宿主端口偶发命中 Windows TCP excluded port range（Hyper-V/WinNAT 动态保留），Docker 发布端口 bind 失败（WSAEACCES），OpenSandbox 包装为 `Egress sidecar container failed to start.`，Job `started_at=null` 直接 failed。建议：dispatcher/executor 对 sandbox 创建失败换 attempt 自动重试一次；透传原始 Docker 错误便于区分端口冲突；`docs/` 补 Windows 排障路径。上游 opensandbox 缺陷已报 [opensandbox-group/OpenSandbox#1702](https://github.com/opensandbox-group/OpenSandbox/issues/1702)（2026-09-02 核查上游 main 未修）。 |
