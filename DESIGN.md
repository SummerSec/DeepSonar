...
- **盲验 Phase 1（#367）**：`verify_finding` 只冻结主体 / location / `artifact_refs`，不下发 maker 的 title/summary/severity；`GraphScope=verify` 默认只投影骨架与引用。`verify_finding` prompt 要求先独立推导再逐项 DIFF，仅 exact match 可 confirm；`confirmed` 另需至少一条带非空 `expected`+`actual` 的 `VerificationEvidence`，路径分叉必须 rework。数值保真 / 矛盾检测 / 对抗挑战留待后续 issue。
- **Fact-first `verify_finding`（#399，#367 follow-up）**：review/test/worker 的复核结果必须作为结构化 Fact 提交，至少携带 `finding_id`、`subject_revision`、`ownership`、`expected`、`actual`、`outcome`；Scheduler 入库时校验目标、版本、同项目/画布归属及 Fact 间冲突。`verify_finding` 不重读 maker 结论或原始 artifacts 作独立复核，只消费通过校验的 Fact 集合；足够且一致的通过 Fact 可直接将 Finding 收口/确认，否则保持未确认并将证据不足、冲突或版本不匹配回弹 Hub。CTF 例：`expected="uid!=0 不能读取 /flag"`、`actual="uid=1000 读到 /flag"`、`outcome="supports"`，仍须绑定同一 `finding_id` / `subject_revision` / `ownership` 才能计入 `verify_finding` 门。
