...
| 长期从 Agentbox 迁移到 OpenSandbox | #162 | **Phase 1/2 契约 + Docker adapter 已落地，Phase 3 overlay 已写入**：`RuntimeHost`/`ensureHost` 是唯一执行边界；五类 CLI（含 Pi / DSH）不按 provider 分流。SDK `@alibaba-group/opensandbox@0.1.11`；server/execd/egress 只认 `name@sha256`。Docker TOML 用官方 schema（bridge、`drop_capabilities=["ALL"]`、PVC）。K8s overlay 选官方默认 **BatchSandbox** + Kata `RuntimeClass=kata-qemu`，namespace 只做基础设施隔离；`agent-sandbox` backend 与 gVisor+egress 组合未验证。`SANDBOX_PROVIDER=opensandbox` 才启用。真实 server PoC：busybox `createMs=2708`；缺合同 / cancel leftover=0；官方 base host `provisionMs=9954` 覆盖文件边界、增量 stdin、PTY、Tab、Ctrl-C、destroy 关终端、`ensureHost`、`network=none` 与 `restricted`（DNS/IPv4/IPv6/CONNECT）隔离、cap-drop/no-new-privs、共享资产只读种子；base/audit/kali 上 `claude/codex/opencode/pi/dsh` 均探测到。官方 amd64 镜像 contract/CLI 探测已覆盖 base/audit/kali 与全部 Chrome/OH 变体。无 LLM 凭据下官方 base 五类 adapter `start`/`encodeInput`/`closeStdin` 已绿（`claude-code`/`codex`/`open-code`/`pi`/`dsh` 均 `started && !notFound && stdinClosed`；`createMs=2634`，合同/cancel leftover=0）。这不是完整 E2E。五类 CLI 真 E2E、arm64、Kata 真机与 Phase 4 删除 Agentbox 仍待完成。 |
