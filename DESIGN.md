...
| 长期从 Agentbox 迁移到 OpenSandbox | #162 | **Phase 1/2 契约 + Docker adapter 已落地，Phase 3 Kata 真机已绿**：`RuntimeHost`/`ensureHost` 是唯一执行边界；五类 CLI（含 Pi / DSH）不按 provider 分流。SDK `@alibaba-group/opensandbox@0.1.11`；server/execd/egress 只认 `name@sha256`。Docker TOML 用官方 schema（bridge、`drop_capabilities=["ALL"]`、PVC）。K8s overlay 选官方默认 **BatchSandbox** + Kata `RuntimeClass=kata-qemu`，namespace 只做基础设施隔离；`agent-sandbox` backend 与 gVisor+egress 组合未验证。`SANDBOX_PROVIDER=opensandbox` 才启用。真实 server PoC：busybox `createMs=2708`；缺合同 / cancel leftover=0；官方 base host `provisionMs=9954` 覆盖文件边界、增量 stdin、PTY、Tab、Ctrl-C、destroy 关终端、`ensureHost`、`network=none` 与 `restricted`（DNS/IPv4/IPv6/CONNECT）隔离、cap-drop/no-new-privs、共享资产只读种子；base/audit/kali 上 `claude/codex/opencode/pi/dsh` 均探测到。官方 amd64 镜像 contract/CLI 探测已覆盖 base/audit/kali 与全部 Chrome/OH 变体。无厂商 LLM 凭据下官方 base 五类 adapter 协议已绿：`claude-code`/`codex`/`open-code`/`pi`/`dsh` 均 `started && inputWritten && steered && stdinClosed`（沙箱内 `127.0.0.1` mock 上游；`createMs=2672`）。CLI launch PoC 现从 adapter JSONL 抽取 session 身份，经 `CLI_SESSION_ADAPTERS.exportSession` 走 `host.run` 归档，并对支持 resume 的 adapter 做同会话恢复；假客户端单测要求五类均 `archived && resumed`。harness 对归档文本调用 `parseAgentSession`。真机官方 base：Claude Code 抽出 session_id、归档 1 个 artifact 并 resume；查看器拆分无换行拼接 JSON 后 `lines=11 parsed=7 items=7`。官方 base 真机 session 归档（生产 HOME + Provider 物化 + loopback mock）：五类均 `archived=true resumed=true` 且查看器可解析（Claude `format=claude-code parsed=6`；Codex `format=codex parsed=12`；OpenCode `format=open-code parsed=1`；Pi `format=pi parsed=8`；DSH `format=dsh parsed=7`）。Codex execd `runAsync` 现用 argv 提示 + `< /dev/null` 打出 `thread.started`；`host.run` 把 execd 分行 log item 用换行拼接，才能发现 `sessions/YYYY/MM/DD/rollout-*.jsonl`。`runRealAgent` 现经 `applyRuntimeOutput` 捕获 session 身份：OpenSandbox 假客户端上五类 CLI 对 `upstream status: 503` 均 `run.retrying` 并按各自契约恢复同会话（Claude `--resume`、Codex `exec resume`、OpenCode `--session`、Pi `--session <sessionFile>`、DSH 复用 `session-${context_id}`），`HTTP 401` 不恢复。SDK create 透传官方 `platform={os,arch}`；`OPEN_SANDBOX_POC_ARCH=amd64|arm64` 跑 `uname -m` 对照，架构不匹配 fail closed。本机真机：`arch=amd64 leftovers=0`；`linux/arm64` 对 amd64-only busybox digest 被 server 拒绝（`platform does not match`），不是静默忽略。官方 arm64 child `sumsec/deepsonar@sha256:90bb6657…` + qemu-user-static 真机 `arch=arm64 leftovers=0`（OpenSandbox 把 platform 套到 execd sidecar；Docker vfs 单平台缓存须先把 execd index 物化为 arm64，跑完再拉回 amd64）。`pnpm ci:smoke:opensandbox -- --case arch` 配合 `OPEN_SANDBOX_POC_ARCH`/`OPEN_SANDBOX_POC_ARCH_IMAGE` 可单独复跑。Dispatcher 按 `provider=opensandbox|agentbox|noop` 记 `deepsonar_sandbox_provision_seconds_*` 与失败计数，供与 Agentbox baseline 对比。DSH 在 `initialize` 前冻结确定性 session id，避免 initialize 级瞬态失败因 `session_missing` 无法恢复。Phase 3 overlay 含 namespace ResourceQuota/LimitRange 与 BatchSandbox container requests/limits；`kustomization.yaml` 只装配基础设施清单，不含 server 模板。`pnpm ci:smoke:opensandbox-cli-control` 可用 `OPEN_SANDBOX_POC_CLI` 选择五类 CLI，缺对应厂商 key 则 skip。重启对账真机：`alive=true reconnect=true dead=true leftovers=0`。`isAlive` 同时看 lifecycle 与 exec 通道并重试；Reaper cleanup 对任何 real provider 生效。OpenSandbox 内 Job Platform API 真机：`AGENT_MODE=real` + `SANDBOX_PROVIDER=opensandbox` 走调度器单例 runner 与 `preparePlatformCapability`；token 在 provision 时写入沙箱 env，worker 只读 `os.environ`（`provisionedEnv=true`），`host.run` 不再覆盖 token。`network=none` 与 `restricted` 均隔离 TEST-NET；`restricted` 只放行 `deepsonar-gateway-proxy`，provision 把该主机名写入沙箱 `/etc/hosts` 并复用路径过滤 sidecar；短期 capability token 提交 `emit_fact`/`emit_finding`/`submit_hub_decision`/`mark_job_done` 均 200，无效 token 401，OpenSandbox API key 不进 Agent env，leftover=0。`pnpm ci:smoke:opensandbox-dispatch` 真机：`claimed=1 provisioned=true cancelled=false leftover=0 tokens=revoked assets=0 status=failed`（冻结项目共享资产经 dispatcher prepare 挂载；无厂商凭据时 executeReal 在 provision 后失败，finally 吊销 capability token 并回收卷；PTY 关闭由 Reaper 同一 `runner.destroy` 路径证明）。`pnpm ci:smoke:opensandbox-cli-control` 是五类 CLI 厂商模型 E2E 入口：默认跑齐所有已配置厂商 key 的 CLI（`OPEN_SANDBOX_POC_CLI` 可只跑一类），并在真实模型路径上要求 steer/follow-up、session 归档、`parseAgentSession` 可解析（`items>0`）与 resume；厂商 key 只进 Scheduler `credentials` + Model Gateway，沙箱只持 Job token；缺 key 则 skip，本地 mock 不能代替。生产部署可通过 `SANDBOX_PROVIDER=opensandbox` 叠加 `docker-compose.opensandbox.prod.yml`，默认 real 仍走 Agentbox。`pnpm ci:smoke:opensandbox-prod` 用临时 env 渲染 `docker-compose.prod.yml` + `docker-compose.real.yml` + overlay，`merged=true provider=opensandbox pinned=true`，不启动生产栈、不写 `deploy/.env`。`pnpm ci:smoke:opensandbox-images` 从 `deploy/runtime-image-registry.json` 取 Docker Hub 不可变 digest，经 adapter provision 后重验 contract 并探测五类 CLI；本机真机九个官方 key 均 leftover=0：base `provisionMs=9367`、audit `12959`、kali-minimal `28610`、openharmony-test/audit/fuzz `14140`/`15092`/`15190`、chrome-audit/test/fuzz `14077`/`15019`/`16542`。SDK create 透传官方 `platform={os,arch}`；`OPEN_SANDBOX_POC_ARCH` 用 `uname -m` 对照且不匹配 fail closed。本机 Docker PoC 已覆盖 `arch=amd64 leftovers=0` 与官方 arm64 child `arch=arm64 leftovers=0`。Dispatcher 按 `opensandbox|agentbox|noop` 记 provision 时延与失败计数。`pnpm ci:smoke:opensandbox-k8s` 在 `OPEN_SANDBOX_POC_K8S=1` 时 fail closed：必须存在 `RuntimeClass=kata-qemu`、namespace/quota，工作负载实际使用该 RuntimeClass，网络隔离 / host-escape / env 不泄漏 / cap-drop+no-new-privs 均由 guest 证据确认，销毁后 leftover pod=0；缺集群则 skip，静态 overlay 不能代替。本机真机：`kata=true isolated=true hostEscapeBlocked=true envClean=true hardLimits=true gatewayAllowed=true denyBlocked=true leftovers=0`（官方 base digest；第二套 server `127.0.0.1:18084` + `docker-compose.opensandbox.k8s.yml`，不停 Phase 2；Kubernetes 不接受 ResourceName=`pids`，`OPEN_SANDBOX_KUBERNETES=1` 时 `OpenSandboxRunner` 对全部 provision（含 reconcile/reaper/dispatch）省略该字段但仍要求冻结 `pidsLimit`；destroy 后等待 Terminating pod 消失；BatchSandbox 模板 `drop: ["ALL"]`；同 namespace 的 `deepsonar-gateway-proxy` Service 经 `/etc/hosts` ClusterIP 可达，兄弟 Service `deepsonar-egress-deny-probe` 被拦）。生产 overlay 把 OpenSandbox 发布在 `127.0.0.1:18080` 并 health-gate scheduler；`pnpm ci:smoke:opensandbox-prod-up` 用独立 compose project 独占拉起 overlay server，打 `/health` 并做鉴权 `list()`；完整 sandbox provision 仍由长期运行的 Phase 2 server 证明（本机 vfs 上冷启动 overlay 的 egress sidecar 会 urlopen timeout）。`pnpm ci:smoke:opensandbox-reconcile` 真机：`requeued=1 orphaned=2 leftover=0 replay=0`（`effect_pending`/running 崩溃 orphan，无效果 preparing 才回 pending）。K8s/Kata 真机同一入口：`kubernetes=true cpu=0.4 domain=127.0.0.1:18084` → `requeued=1 orphaned=2 leftover=0 replay=0`。该开关下并发 sandbox CPU 映射为 `400m`，LimitRange 默认 request 为 50m，共享资产走 `NoopSharedAssetsVolumeManager`（K8s overlay 不挂 docker.sock）。`pnpm ci:smoke:opensandbox-reaper` 真机：`timeout=1 orphan=1 live=1 leftover=0 tokens=revoked pty=closed assets=0 aliveAfter={timeout:false,orphan:false,live:true}`（先在健康 running Job 上铸造 capability token、打开 PTY 并挂只读共享资产，再老化 timeout/orphan 行；未到期 Job 的 token、PTY、卷均保留）。K8s/Kata 真机同一入口：`kubernetes=true cpu=0.3 memoryMiB=256 domain=127.0.0.1:18084` → 同样 `timeout=1 orphan=1 live=1 leftover=0 tokens=revoked pty=closed`（K8s 跳过 Docker 卷，namespace leftover pod=0）。镜像契约解析、共享资产标签、保留工作区读路径、human inbox writer 与 inbox 路径解析已抽到 `runtime-shared.ts`；有界工作区文件读取与 human inbox 写入在 `runtime-docker.ts`（`readDockerWorkspaceFile` / `writeDockerHumanInboxFile`）（`shared-assets-volume.integration.test.ts` 不再 import `agentbox.ts`）（`human-inbox.test.ts` 不再 import `agentbox.ts`）；控制台侧栏读取 `GET /health.opensandbox`，仅在 `ok/error/unconfigured` 时显示探测结果，不回显错误正文。`runRealAgent` / CLI 物化 / 语义工具状态已抽到 `runtime-agent.ts`；Docker CLI 与 Model Gateway sidecar 已抽到 `runtime-docker.ts` / `runtime-gateway.ts`。OpenSandbox adapter、Agent 执行层和 Gateway sidecar 都不再 import `agentbox.ts`。主 barrel 不再 re-export `AgentboxRunner`；过渡实现只经 `@deepsonar/runtime-sandbox/agentbox` 子路径按需加载，`SANDBOX_PROVIDER=opensandbox` 时 Scheduler 不加载 `agentbox-sdk`。Dispatcher 的本机 `docker inspect` 只在 `local-docker` 执行；OpenSandbox 冷宿主不再因调度器缺层拒绝 Job，合同/digest 仍在 provision 后重验。`SANDBOX_PROVIDER=opensandbox` 时 `GET /readiness` 与 Dispatcher claim 会探测 OpenSandbox server（鉴权 `list()`，缺 key / 不可达 fail closed），不再把本机 Docker 水位当成唯一基础设施门。`pnpm ci:smoke:opensandbox-prod-stack` 用进程内 Scheduler 打 `/readiness` + `/health`，要求 `OPENSANDBOX_SERVER_READY` 且 leftover=0，不停 Phase 2 server、不 provision。`pnpm ci:smoke:opensandbox-prod-compose` 用 `docker-compose.opensandbox.host.yml` 本地构建生产 scheduler+web 镜像；postgres+scheduler+silo 走 host 网络连 `127.0.0.1`（本机 compose bridge 在 iptables-legacy FORWARD=DROP 时丢包）和已运行的 Phase 2 server，`BLOB_STORE=s3` 指向 loopback Silo。`pnpm ci:smoke:opensandbox-prod-official` 在把 legacy FORWARD 设为 ACCEPT 后，用官方 `docker-compose.prod.yml` + `real.yml` + `opensandbox.prod.yml` 走 bridge 拉起 scheduler+web+silo+overlay OpenSandbox（隔离 gateway 网名，不停 Phase 2 server）。`GET /health` 在 `SANDBOX_PROVIDER=opensandbox` 时把鉴权探测写入 `opensandbox.level/ready`，server 不可达则 `ready=false`（HTTP 仍 200）。核心 CI 渲染 prod+real+overlay compose merge。这仍不是五类 CLI 厂商模型完整 E2E。Phase 4 删除 Agentbox 仍待完成。 |
...
| `packages/runtime-sandbox` | SandboxRunner / RuntimeHost（Agentbox 过渡 + OpenSandbox adapter） |
