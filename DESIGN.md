...
- **任务创建原子边界（#203）**：`POST /projects/:id/tasks` 在一个 `sql.begin` 内完成目标/Finding 协议/compose 种子校验、有效网络策略与 Hub 运行快照冻结、Canvas/root/只读种子投影、入口 `hub_reason` Job、Job 图节点/边及精确共享资产版本链接。runtime image、Credential、网络或任一快照步骤失败会整体回滚，不留下 root-only Canvas；稳定错误码区分无 trusted 版本、所选版本、宿主平台、registry 通道/ref 与其它快照不可用。`task.create` 审计使用独立连接，因此只在业务事务提交后写入，并只引用已提交的 Canvas/Job；失败创建不写审计。
...
| 任务创建原子提交 | #203 | **已完成**：人工任务的目标/种子、网络与 Hub 快照、Canvas/root/seed、Job/图边和共享资产链接同事务提交；镜像/凭据失败无空画布、无失败审计，并以稳定 runtime/snapshot 错误码返回 |
