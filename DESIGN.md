...
**#590 叠层（与 #576 对齐）**：每个 `used_fact_ids` 必须在确认前 EXISTS 解析到真实 Fact（finding 归属、ownership、来源 Job ≠ Finding 原始 Job）；悬空引用一律阻断并 rework——缺 Artifact 四表行仍不单独构成悬空（#577）。对 `security.vulnerability` / `security.misconfig` / `security.secret`，独立 review≥1、合格 test≥1、`missing_evidence == []` 为 confirmed 必要断言，配对缺口升为 **required_missing**，不得抹空 missing 放行。`emit_finding` 可选 `impact`（`attacker_entry` / `victim_resource` / `impact_flow` / `preconditions` / `known_issue_refs` / `impact_waiver`）；上述安全 profile 的 confirmed 要求非空 `impact_flow` 或显式 `impact_waiver`，并需 `attempted_refutation` 与合格 test 的 `artifact_refs`。默认 `require_scoring_for_profiles` 含 `security.vulnerability`；location/summary 中的 TODO/crbug/CVE 自动打 `known_issue` 标签。
