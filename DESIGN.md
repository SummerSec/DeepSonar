...
| 长期从 Agentbox 迁移到 OpenSandbox | #162 | **Phase 1 已落地，Phase 2 adapter 可测**：`packages/runtime-sandbox` 现有 provider-neutral `RuntimeHost` / `RuntimeProcess` / `RuntimeResource`；五类 CLI adapter、`runRealAgent`、session archive、human inbox 与 reconcile/cleanup 不再引用 Agentbox/OpenSandbox SDK 类型。`OpenSandboxRunner` 用注入的 client 做 Docker PoC（`SANDBOX_PROVIDER=opensandbox`），硬限制缺失 fail closed，`none/restricted/egress` 映射到 networkPolicy，TTL 由 Scheduler 持有。real 默认仍走 Agentbox 实现这些契约。Phase 3 Kubernetes/Kata 与 Phase 4 删除 Agentbox 仍待全部门禁。 |
