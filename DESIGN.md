...
| 官方清单 fallback 不自残 | #191 | **已完成**：远程清单不可达时 `fallback=true` 只插入缺失版本，不 rename / 不改已有 `image_ref` / 不 revoke。官方可信版本的准入扫描失败保持 trusted 并写审计；发行版 CRITICAL/secret 也不再自动 revoke（见 #259）。信任清空时打 error 级日志并写 `runtime_image.official_trust_empty` 审计。 |
...
| 官方 runtime 不被 Trivy CRITICAL 复扫吊销 | #259 | **已完成**：`image-admission` 对官方 catalog 记录 CVE/secret 但不自动 `revoked`；第三方仍 0 CRITICAL / 0 secret 且扫描通过后 quarantined。Job 选择器在没有 trusted 版本时返回 `409 RUNTIME_IMAGE_REVOKED` / `RUNTIME_IMAGE_NOT_TRUSTED`，不再伪装成 `PLATFORM_UNAVAILABLE`。目录同步恢复因 `admission policy failed` 被误吊销的官方 digest 并写审计；warmup `/health.runtime_images.error_code` 对官方默认镜像无 trusted 打 error。 |
