...
| Pi Coding Agent RPC Runtime Adapter 与 Capability API | #140/#197/#193/#194/#336 | **已落地**：Pi 固定使用 `pi --mode rpc --no-approve` 的严格 LF JSONL 协议，平台控制通过 Job 级 HTTP Capability API，不物化或注入 MCP；`agent_settled` 只提供运行时静止信号，Job 成功仍须经过 `mark_job_done` 完成门。`get_state` 返回的精确 `sessionFile` 用于恢复，暂态错误最多同会话重试三次；模型配置经 Gateway 物化为 `models.json` + `auth.json`，`--model` 使用冻结的 `provider/model`（`agent-default-model.provider` 随模型一起冻结，不把裸模型 ID 插入每个 Provider）。空 content/零 usage、`stopReason=error/aborted`、`auto_retry_end.success=false`、`compaction_end.result=null` 与 `message_update` error 均视为协议/上游失败，不得被后续 `agent_settled` 报成成功。工具事件按 0.84.4 顶层 `toolCallId`/`args`/`isError`/`partialResult` 解析。仓库 Skill 安装失败阻断 Job。当前三类治理 CLI 均声明 `interactiveTerminal`；Web 对未声明能力的快照显式提示「该 CLI 暂不支持交互终端」。项目 `.pi` 不自动加载，默认 `--no-extensions`，受治理扩展才通过冻结配置显式 `--extension` 加载。 |
